...
class DataCubeStage(PipelineStage):
    ...
    def _extract_climate_metrics(self, dataset: xr.Dataset) -> Dict[str, float]:
        ...
        if 'T_surf' in dataset:
            ...
            metrics['equatorial_temperature'] = float(temp_data.isel(lat=len(temp_data.lat)//2).mean())
...
class SpectrumStage(PipelineStage):
    ...
    def _process_impl(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        wavelength, flux = generate_spectrum(
            column_properties, 
            planet=data,
            resolution=self.config.spectral_resolution
        )
    ...
    def _calculate_spectral_features(self, wavelength: np.ndarray, flux: np.ndarray) -> Dict[str, float]:
...
class AdvancedPipeline:
    ...
    def process_planet(self, planet_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for stage in self.stages:
            try:
                ...
                logger.error(f"Pipeline failed at stage {stage.name} for planet {planet_data.get('name', 'unknown')}: {e}")
    ...
    def process_batch(self, planets: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        if self.config.num_workers > 1:
            ...
            with ThreadPoolExecutor(max_workers=self.config.num_workers) as executor:
                futures = {executor.submit(self.process_planet, planet): planet for planet in planets}
                
                for future in tqdm(as_completed(futures), total=len(futures), desc="Processing planets"):
                    try:
                        result = future.result(timeout=self.config.timeout_seconds)
                        results.append(result)
                    except Exception as e:
                        planet = futures[future]
                        logger.error(f"Failed to process planet {planet.get('name', 'unknown')}: {e}")
    ...
    def _save_results(self, results: List[Dict[str, Any]], stats: Dict[str, Any]):
        ...
        # Save results
        results_file = self.config.output_dir / f"results_{self.config.mode}_{int(time.time())}.json"
...
def main():
    ...
    parser.add_argument("--mode", default="datacube", choices=["scalar", "datacube", "joint", "spectral"],
                       help="Pipeline mode")
