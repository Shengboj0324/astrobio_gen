...
from utils.cache import put as cache_put
...
def _call_psg(cfg: str) -> str:
    ...
    try:
        ...
        resp = requests.post(
            api_url, files={"file": ("config.xml", cfg)}, data={"key": key}, timeout=120
        )
        resp.raise_for_status()

        # Log successful access for URL health monitoring
        if _psg_interface.url_system and hasattr(
            _psg_interface.url_system, "log_successful_access"
        ):
...
def get_spectrum(
    atmos: dict[str, float], planet: dict, instrument="JWST-NIRSpec", R=1000
