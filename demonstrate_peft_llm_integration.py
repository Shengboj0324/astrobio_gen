...
try:
    from api.llm_endpoints import PlanetParameters, QARequest, RationaleRequest, VoiceOverRequest
...
class PEFTLLMIntegrationDemo:
        ...
        if MODELS_AVAILABLE:
            try:
                ...
                self.surrogate_model = SurrogateTransformer(
                    dim=128, depth=4, heads=4, n_inputs=8, mode="scalar"  # Smaller for demo
                ).to(self.device)
    ...
    async def _test_plain_english_rationale(self):
        ...
        for planet_data in test_planets:
            ...
            try:
                ...
                start_time = time.time()
                analysis = await self.llm_coordinator.generate_comprehensive_analysis(
                    surrogate_outputs
                )
                ...
                result = {
                    ...
                    "surface_temperature_c": structured_data.get("surface_temperature", 273.15)
                    - 273.15,
                    "confidence": (
                        "High" if structured_data.get("uncertainty_sigma", 0.2) < 0.1 else "Medium"
                    ),
                }
                ...
                rationale_results.append({"planet_name": planet_data["name"], "error": str(e)})
    ...
    async def _test_interactive_qa(self):
        ...
        for question in test_questions:
            ...
            try:
                ...
                answer = await self.llm_coordinator.answer_question(
                    question, sample_surrogate_outputs
                )
                ...
                result = {
                    ...
                    "includes_sources": "KEGG" in answer
                    or "GCM" in answer
                ...
                qa_results.append({"question": question, "error": str(e)})
        ...
        self.results["interactive_qa"] = {
            ...
            "avg_response_time_ms": np.mean(
                [r.get("response_time_ms", 0) for r in qa_results if "response_time_ms" in r]
            ),
            "avg_answer_length": np.mean(
                [r.get("answer_length", 0) for r in qa_results if "answer_length" in r]
            ),
        }
    ...
    async def _test_voice_over_generation(self):
        ...
        scenarios = [
            {
                ...
                "planet": PlanetParameters(
                    radius_earth=1.0, mass_earth=1.0, insolation=1.0, stellar_teff=5778
                ),
                "target_duration": 60,
            },
            {
                ...
                "planet": PlanetParameters(
                    radius_earth=0.8, mass_earth=0.6, insolation=0.4, stellar_teff=3500
                ),
                "target_duration": 45,
            },
        ]
        ...
        for scenario in scenarios:
            ...
            try:
                ...
                result = {
                    ...
                    "duration_accuracy": abs(estimated_duration - scenario["target_duration"])
                ...
                logger.info(
                    f"⏱️ Duration: {estimated_duration:.1f}s (target: {scenario['target_duration']}s)"
                )
                logger.info(f"🎬 Script preview: {script[:100]}...")

            except Exception as e:
                logger.error(f"❌ Failed to generate script for {scenario['name']}: {e}")
                voice_over_results.append({"scenario": scenario["name"], "error": str(e)})
        ...
        self.results["voice_over_generation"] = {
            ...
            "avg_generation_time_ms": np.mean(
                [
                    r.get("generation_time_ms", 0)
                    for r in voice_over_results
                    if "generation_time_ms" in r
                ]
            ),
            "avg_duration_accuracy": np.mean(
                [
                    r.get("duration_accuracy", 1.0)
                    for r in voice_over_results
                    if "duration_accuracy" in r
                ]
            ),
        }
        ...
        if MODELS_AVAILABLE and self.surrogate_model and self.cnn_model:
            try:
                ...
                surrogate_data = {
                    "predictions": {
                        "habitability": float(
                            surrogate_output.get("habitability", torch.tensor([0.7]))[0]
                        ),
                        "surface_temp": float(
                            surrogate_output.get("surface_temp", torch.tensor([290.0]))[0]
                        ),
                        "atmospheric_pressure": float(
                            surrogate_output.get("atmospheric_pressure", torch.tensor([1.0]))[0]
                        ),
                    },
                    "uncertainty": 0.1,
                }
                ...
                coordination_results.append(
                    {
                        ...
                        "surrogate_output_shape": (
                            list(surrogate_output["habitability"].shape)
                            if "habitability" in surrogate_output
                            else []
                        ),
                ...
                coordination_results.append({"coordination_successful": False, "error": str(e)})
        else:
    ...
    async def _benchmark_performance(self):
        ...
        for task, stats in performance_stats.items():
            logger.info(
                f"🎯 {task}: {stats['avg_time_ms']:.1f}ms avg (±{stats['std_time_ms']:.1f}ms)"
            )
    ...
    async def _get_surrogate_predictions(self, planet_params: PlanetParameters) -> Dict[str, Any]:
        ...
        if MODELS_AVAILABLE and self.surrogate_model:
            try:
                ...
                return {"predictions": outputs, "uncertainty": torch.tensor(0.1)}
    ...
    def _print_summary(self):
        ...
        logger.info(
            f"❓ Interactive Q&A: {qa.get('success_count', 0)}/{qa.get('test_count', 0)} tests passed"
        )
        logger.info(
            f"🎤 Voice-Over Generation: {voice_over.get('success_count', 0)}/{voice_over.get('test_count', 0)} tests passed"
        )
        ...
        logger.info(
            f"   LLM Functionality: {'✅' if validation.get('llm_functionality') else '❌'}"
        )
