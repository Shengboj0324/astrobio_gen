...
class RationaleRequest(BaseModel):
    ...
    target_audience: str = Field(default="scientific", description="Target audience: scientific, general, technical")
...
class VoiceOverRequest(BaseModel):
    ...
    style: str = Field(default="scientific", description="Presentation style: scientific, educational, conference")
...
async def interactive_qa(
    ...
    try:
        ...
        answer_text = await coordinator.answer_question(
            request.question, 
            surrogate_outputs
        )
...
async def generate_voice_over(
    ...
    try:
        ...
        if request.include_audio and TTS_AVAILABLE:
            audio_data = await _generate_tts_audio(
                script_text, 
                request.voice_settings or {}
            )
...
async def llm_health_check():
    ...
    try:
        ...
        return {
            ...
            "knowledge_base_ready": coordinator.peft_llm.knowledge_retriever.knowledge_index is not None,
...
async def _get_answer_sources(question: str, coordinator: LLMSurrogateCoordinator) -> List[Dict[str, str]]:
    """Get sources used in Q&A answers"""
    try:
        docs = await coordinator.peft_llm.knowledge_retriever.retrieve_relevant_docs(question, max_docs=3)
...
async def _generate_tts_audio(text: str, voice_settings: Dict[str, Any]) -> bytes:
    ...
    if not TTS_AVAILABLE:
        raise HTTPException(
            ...
            detail="TTS functionality not available"
        )
