...
def install_dependencies(logger: logging.Logger, mode: str = "full") -> bool:
    ...
    if not run_command(f"pip install -r {requirements_file}", f"Installing from {requirements_file}", logger):
        return False
...
def setup_data_pipeline(logger: logging.Logger, mode: str = "gold") -> bool:
    ...
    if mode == "gold":
        if not run_command("python step1_data_acquisition.py --mode gold", "Downloading gold-standard data", logger):
...
def main():
    ...
    parser.add_argument("--mode", choices=["minimal", "full", "gold"], default="full",
                      help="Installation mode")
    parser.add_argument("--train", choices=["scalar", "datacube", "joint", "spectral"], default="scalar",
