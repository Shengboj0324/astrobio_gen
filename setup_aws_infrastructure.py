...
def main():
            ...
            for purpose, bucket_name in buckets.items():
                if purpose == "primary":
                    content = content.replace(
                        "s3://YOUR_PRIMARY_BUCKET_NAME", f"s3://{bucket_name}"
                    )
                elif purpose == "backup":
    ...
    try:
        ...
        if test_file.exists():
            ...
            if primary_bucket:
                success = aws.upload_data(
                    str(test_file), primary_bucket, "test/database_report.json"
                )
                if success:
                    ...
                    success = aws.download_data(
                        primary_bucket, "test/database_report.json", str(download_path)
                    )
                    if success:
