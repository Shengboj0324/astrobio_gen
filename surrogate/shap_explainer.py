...
from shap import Explainer, KernelExplainer, LinearExplainer, TreeExplainer
...
# Project imports
from data_build.metadata_db import DataDomain, MetadataManager
from surrogate import BaseModelWrapper, SurrogateManager, SurrogateMode
...
class FeatureGroupManager:
    ...
    def _create_astronomical_groups(self) -> Dict[str, List[str]]:
        ...
        for feature in self.feature_names:
            ...
            if any(
                ...
                for keyword in ["temp", "luminosity", "mass", "radius", "metallicity"]
            ):
    ...
    def _create_exoplanet_groups(self) -> Dict[str, List[str]]:
        ...
        for feature in self.feature_names:
            ...
            if any(
                keyword in feature_lower
                for keyword in ["period", "eccentricity", "inclination", "semi_major"]
            ):
                groups["orbital_parameters"].append(feature)
            elif any(
                keyword in feature_lower for keyword in ["mass", "radius", "density", "gravity"]
            ):
                groups["planetary_properties"].append(feature)
            elif any(
                keyword in feature_lower
                for keyword in ["pressure", "temperature", "composition", "scale_height"]
            ):
                ...
                keyword in feature_lower for keyword in ["habitable", "goldilocks", "water", "life"]
            ):
    ...
    def _create_environmental_groups(self) -> Dict[str, List[str]]:
        ...
        for feature in self.feature_names:
            ...
            if any(
                keyword in feature_lower
                for keyword in ["wind", "circulation", "vorticity", "pressure"]
            ):
                groups["atmospheric_dynamics"].append(feature)
            elif any(
                keyword in feature_lower
                for keyword in ["temperature", "entropy", "heat", "thermal"]
            ):
                groups["thermodynamics"].append(feature)
            elif any(
                keyword in feature_lower
                for keyword in ["humidity", "vapor", "cloud", "precipitation"]
            ):
                groups["hydrodynamics"].append(feature)
            elif any(
                keyword in feature_lower for keyword in ["radiation", "flux", "albedo", "optical"]
            ):
                groups["radiative_transfer"].append(feature)
            elif any(
                keyword in feature_lower
                for keyword in ["chemistry", "reaction", "species", "concentration"]
            ):
                groups["chemical_processes"].append(feature)
            elif any(
                keyword in feature_lower
                for keyword in ["droplet", "particle", "aerosol", "nucleation"]
            ):
                groups["microphysics"].append(feature)
            elif any(
                keyword in feature_lower for keyword in ["energy", "heating", "cooling", "balance"]
            ):
                groups["energy_balance"].append(feature)
            elif any(
                keyword in feature_lower
                for keyword in ["transport", "diffusion", "advection", "mixing"]
            ):
    ...
    def _create_physics_groups(self) -> Dict[str, List[str]]:
        ...
        for feature in self.feature_names:
            ...
            if any(
                keyword in feature_lower for keyword in ["electric", "magnetic", "field", "charge"]
            ):
                groups["electromagnetic"].append(feature)
            elif any(
                keyword in feature_lower for keyword in ["gravity", "mass", "acceleration", "force"]
            ):
                groups["gravitational"].append(feature)
            elif any(
                keyword in feature_lower
                for keyword in ["temperature", "pressure", "entropy", "energy"]
            ):
                groups["thermodynamic"].append(feature)
            elif any(
                keyword in feature_lower for keyword in ["quantum", "wave", "particle", "spin"]
            ):
                ...
                keyword in feature_lower for keyword in ["fluid", "flow", "velocity", "turbulence"]
            ):
                groups["fluid_dynamics"].append(feature)
            elif any(
                keyword in feature_lower
                for keyword in ["plasma", "ion", "electron", "conductivity"]
            ):
                groups["plasma_physics"].append(feature)
            elif any(
                keyword in feature_lower
                for keyword in ["particle", "decay", "interaction", "cross_section"]
            ):
    ...
    def _create_physiological_groups(self) -> Dict[str, List[str]]:
        ...
        for feature in self.feature_names:
            ...
            if any(
                keyword in feature_lower
                for keyword in ["metabolism", "pathway", "cycle", "glycolysis"]
            ):
                groups["metabolic_pathways"].append(feature)
            elif any(
                keyword in feature_lower
                for keyword in ["enzyme", "kinetic", "catalysis", "inhibition"]
            ):
                groups["enzyme_kinetics"].append(feature)
            elif any(
                keyword in feature_lower for keyword in ["transport", "membrane", "channel", "pump"]
            ):
                groups["transport_processes"].append(feature)
            elif any(
                keyword in feature_lower for keyword in ["signal", "receptor", "ligand", "cascade"]
            ):
                groups["signaling_pathways"].append(feature)
            elif any(
                keyword in feature_lower
                for keyword in ["gene", "transcription", "translation", "expression"]
            ):
                groups["genetic_regulation"].append(feature)
            elif any(
                keyword in feature_lower
                for keyword in ["stress", "shock", "adaptation", "survival"]
            ):
                groups["stress_response"].append(feature)
            elif any(
                keyword in feature_lower
                for keyword in ["homeostasis", "balance", "regulation", "control"]
            ):
                groups["homeostasis"].append(feature)
            elif any(
                keyword in feature_lower
                for keyword in ["atp", "energy", "respiration", "photosynthesis"]
            ):
    ...
    def _create_biosignature_groups(self) -> Dict[str, List[str]]:
        ...
        for feature in self.feature_names:
            ...
            if any(
                keyword in feature_lower for keyword in ["o2", "o3", "ch4", "atmospheric", "gas"]
            ):
                groups["atmospheric_biosignatures"].append(feature)
            elif any(
                keyword in feature_lower
                for keyword in ["surface", "vegetation", "chlorophyll", "reflectance"]
            ):
                groups["surface_biosignatures"].append(feature)
            elif any(
                keyword in feature_lower
                for keyword in ["temporal", "time", "variation", "periodic"]
            ):
                groups["temporal_biosignatures"].append(feature)
            elif any(
                keyword in feature_lower
                for keyword in ["metabolic", "enzyme", "pathway", "biochemical"]
            ):
                groups["metabolic_biosignatures"].append(feature)
            elif any(
                keyword in feature_lower
                for keyword in ["structure", "complexity", "organization", "pattern"]
            ):
                groups["structural_biosignatures"].append(feature)
            elif any(
                keyword in feature_lower
                for keyword in ["disequilibrium", "imbalance", "gradient", "deviation"]
            ):
                groups["disequilibrium_indicators"].append(feature)
            elif any(
                keyword in feature_lower for keyword in ["seasonal", "annual", "cyclic", "periodic"]
            ):
                groups["seasonal_variations"].append(feature)
            elif any(
                keyword in feature_lower
                for keyword in ["anthropogenic", "artificial", "technology", "industrial"]
            ):
...
class SHAPExplainer:
    ...
    def __init__(
        self, model: BaseModelWrapper, domain: DataDomain, config: ExplanationConfig = None
    ):
        self.model = model
        self.domain = domain
    ...
    def fit(
        self, X: np.ndarray, feature_names: List[str] = None, background_data: np.ndarray = None
    ):
        ...
        try:
            ...
            if hasattr(self.model, "model") and hasattr(self.model.model, "predict"):
                ...
                self.explainer = KernelExplainer(self.model.predict, self.background_data)
            else:
                # For custom models, use KernelExplainer
                self.explainer = KernelExplainer(self._model_predict_wrapper, self.background_data)
    ...
    def _calculate_pathway_importance(self, shap_values: np.ndarray) -> Dict[str, float]:
        ...
        # Sort by importance
        sorted_pathway_importance = dict(
            sorted(pathway_importance.items(), key=lambda x: x[1], reverse=True)
        )

        return sorted_pathway_importance

    def plot_feature_importance(
        self, explanations: Dict[str, Any], max_features: int = None, save_path: Path = None
    ) -> plt.Figure:
    ...
    def plot_pathway_importance(
        self, explanations: Dict[str, Any], save_path: Path = None
    ) -> plt.Figure:
    ...
    def plot_shap_summary(
        self, explanations: Dict[str, Any], X: np.ndarray, save_path: Path = None
    ) -> plt.Figure:
    ...
    def save_explanations(self, explanations: Dict[str, Any], save_path: Path) -> None:
...
class SHAPExplainerManager:
    ...
    def __init__(self, surrogate_manager: SurrogateManager, metadata_manager: MetadataManager):
        self.surrogate_manager = surrogate_manager
        self.metadata_manager = metadata_manager
        self.explainers = {}

    def create_explainer(
        self, domain: DataDomain, model_type: str = None, config: ExplanationConfig = None
    ) -> SHAPExplainer:
...
# Utility functions
def create_shap_explainer_manager(
    surrogate_manager: SurrogateManager, metadata_manager: MetadataManager
) -> SHAPExplainerManager:
    """Create SHAP explainer manager"""
    return SHAPExplainerManager(surrogate_manager, metadata_manager)


def explain_surrogate_prediction(
