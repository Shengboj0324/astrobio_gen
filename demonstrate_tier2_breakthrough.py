...
try:
    ...
    from models.causal_discovery_ai import CausalDiscoveryAI, create_causal_discovery_system
...
class Tier2BreakthroughDemo:
    ...
    async def _initialize_tier2_systems(self):
        ...
        try:
            ...
            self.climate_generator = create_climate_generator(
                spatial_resolution=(32, 64, 16), enable_physics=True  # Reduced for demo
            )
        ...
        try:
            ...
            if self.causal_discovery:
                ...
                discovery_summary = {
                    ...
                    "confidence_score": (
                        np.mean([h.confidence_score for h in hypotheses]) if hypotheses else 0
                    ),
                }
            ...
            logger.info(
                f"✅ Discovered {discovery_summary['causal_edges_found']} causal relationships"
            )
            ...
            self.metrics["causal_discovery"] = {
                ...
                "avg_hypothesis_novelty": np.mean(
                    [h["novelty_score"] for h in breakthrough_hypotheses]
                ),
    ...
    async def _demonstrate_autonomous_discovery_breakthrough(self):
        ...
        try:
            ...
            if self.autonomous_discovery:
                ...
                research_summary = {
                    ...
                    "insights_generated": len(
                        research_results["results"].get("insights", {}).get("insights", [])
                    ),
                    "research_duration": str(research_results.get("duration", timedelta(0))),
                    "research_impact": research_results["results"]
                    .get("insights", {})
                    .get("research_impact", {}),
                }
            else:
            ...
            logger.info(
                f"✅ Autonomous research completed: {research_summary['insights_generated']} insights"
            )
            ...
            adaptation_success_rate = sum(s["success"] for s in adaptation_scenarios) / len(
                adaptation_scenarios
            )
            avg_adaptation_time = np.mean(
                [float(s["adaptation_time"].split()[0]) for s in adaptation_scenarios]
            )

            logger.info(
                f"⚡ Real-time adaptation: {adaptation_success_rate:.1%} success, {avg_adaptation_time:.1f}s avg time"
            )
    ...
    def _calculate_discovery_potential(self) -> Dict[str, float]:
        ...
        breakthrough_discovery = (
            exploration_capability + pattern_recognition + predictive_power
        ) / 3
    ...
    def _create_complex_astrobiology_dataset(self) -> pd.DataFrame:
        ...
        water_vapor = np.exp((surface_temp - 273) / 50) * (
            1 + stellar_metallicity
        ) + np.random.normal(0, 0.1, n_samples)
        co2_concentration = surface_pressure * 400 * np.random.lognormal(0, 1, n_samples)
        o2_concentration = np.maximum(
            0,
            (surface_temp - 200) / 100 + stellar_metallicity + np.random.normal(0, 0.5, n_samples),
        )

        # Magnetic field effects
        magnetic_field_strength = (
            planet_mass * planet_radius * np.random.lognormal(0, 0.5, n_samples)
        )
        ...
        habitability_score = (
            temp_habitability * pressure_habitability * water_habitability * stability_habitability
        ) + np.random.normal(0, 0.05, n_samples)
        habitability_score = np.clip(habitability_score, 0, 1)

        # Biosignature indicators
        biosignature_strength = (
            o2_concentration * water_vapor * habitability_score
            + np.random.normal(0, 0.1, n_samples)
        )
    ...
    async def _generate_breakthrough_report(self):
        ...
        logger.info(
            f"   Integration Success: {'Yes' if self.metrics.get('integration', {}).get('integration_success_rate', 0) > 0.8 else 'Partial'}"
        )

        if "breakthrough_potential" in self.metrics:
            breakthrough_score = self.metrics["breakthrough_potential"].get(
                "overall_breakthrough_score", 0
            )
            breakthrough_category = self.metrics["breakthrough_potential"].get(
                "breakthrough_category", "Unknown"
            )
