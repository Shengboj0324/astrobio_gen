...
class BenchmarkPlanet:
    ...
    name: str
    parameters: List[float]  # [radius, mass, period, insolation, st_teff, st_logg, st_met, host_mass]
...
class BenchmarkSuite:
    ...
    def _validate_benchmark_planets(
        ...
        for planet in self.benchmark_planets:
            ...
            with torch.no_grad():
                ...
                if include_uncertainty and hasattr(model, 'predict_with_uncertainty'):
                    ...
                    planet_result["temperature_uncertainty"] = float(uncertainty_outputs["surface_temp_std"].item())
                    planet_result["habitability_uncertainty"] = float(uncertainty_outputs["habitability_std"].item())
        ...
        aggregate_metrics = {
            ...
            "success_rate": sum(1 for p in results.values() if p["within_tolerance"]) / len(results)
        }
    ...
    def _validate_uncertainty_calibration(self, model: torch.nn.Module) -> Dict[str, Any]:
        ...
        def gaussian_crps(pred, std, obs):
            z = (obs - pred) / std
            return std * (z * (2 * stats.norm.cdf(z) - 1) + 2 * stats.norm.pdf(z) - 1/np.sqrt(np.pi))
        
        crps_scores = [gaussian_crps(p, s, t) for p, s, t in zip(predictions, uncertainties, targets)]
        ...
        uncertainty_results = {
            ...
            "uncertainty_reliability": {
                "well_calibrated": abs(coverage_68 - 0.68) <= 0.05 and abs(coverage_95 - 0.95) <= 0.05,
    ...
    def _compute_overall_assessment(self, validation_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Extract key metrics
        benchmark_success_rate = validation_results["benchmark_validation"]["aggregate_metrics"]["success_rate"]
        benchmark_mae = validation_results["benchmark_validation"]["aggregate_metrics"]["mean_absolute_error"]
        ...
        performance_time = validation_results["performance_metrics"]["single_inference"]["mean_time"]
        performance_target_met = validation_results["performance_metrics"]["single_inference"]["target_met"]
        
        # Overall grades
        benchmark_grade = "A" if benchmark_success_rate >= 0.9 and benchmark_mae <= 5.0 else \
                         "B" if benchmark_success_rate >= 0.8 and benchmark_mae <= 10.0 else \
                         "C" if benchmark_success_rate >= 0.7 else "F"
        
        physics_grade = "A" if physics_violation_rate <= 0.01 else \
                       "B" if physics_violation_rate <= 0.05 else \
                       "C" if physics_violation_rate <= 0.1 else "F"
        
        performance_grade = "A" if performance_target_met else "B"
        
        # NASA readiness assessment
        nasa_ready = (
            benchmark_grade in ["A", "B"] and
            physics_grade in ["A", "B"] and
            performance_grade == "A" and
            benchmark_r2 >= 0.9
        )
    ...
    def _generate_recommendations(self, validation_results: Dict[str, Any]) -> List[str]:
        ...
        if benchmark_success_rate < 0.8:
            recommendations.append("Improve benchmark planet accuracy with additional training data")
        ...
        if "uncertainty_validation" in validation_results:
            ...
            if not uncertainty_results.get("uncertainty_reliability", {}).get("well_calibrated", True):
                recommendations.append("Improve uncertainty calibration with more diverse training data")
    ...
    def _generate_validation_report(self, results: Dict[str, Any]):
        ...
        with open(report_file, 'w') as f:
            ...
            f.write(f"- **Benchmark Success Rate:** {overall['key_metrics']['benchmark_success_rate']:.1%}\n")
            f.write(f"- **Physics Violation Rate:** {overall['key_metrics']['physics_violation_rate']:.1%}\n")
            f.write(f"- **Inference Time:** {overall['key_metrics']['inference_time_seconds']:.3f}s\n\n")
            ...
            for planet, result in benchmark_results.items():
                ...
                f.write(f"| {planet} | {result['predicted_temperature']:.1f} | "
            ...
            f.write(f"- **Single Inference:** {perf['single_inference']['mean_time']:.3f}s ± {perf['single_inference']['std_time']:.3f}s\n")
            f.write(f"- **Target Met:** {'✅' if perf['single_inference']['target_met'] else '❌'} (<0.4s)\n")
