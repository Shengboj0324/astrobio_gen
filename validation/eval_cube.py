...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("eval_cube.log"), logging.StreamHandler()],
)
...
class PhysicsValidator:
        ...
        try:
            ...
            if q_H2O is not None:
                ...
                if np.sum(valid_mask) > 100:  # Enough data points
                    correlation = np.corrcoef(temp_flat[valid_mask], humidity_flat[valid_mask])[
                        0, 1
                    ]
            ...
            results = {
                ...
                "thermodynamic_consistent": temp_range_valid
                and gradient_valid
                and clausius_clapeyron_valid,
            }
    ...
    def validate_cube(self, cube: xr.Dataset) -> Dict[str, Any]:
        ...
        all_valid = all(
            result.get("energy_balance_valid", True)
            or result.get("hydrostatic_valid", True)
            or result.get("mass_conservation_valid", True)
            or result.get("thermodynamic_consistent", True)
...
class BenchmarkEvaluator:
    ...
    def evaluate_benchmark_planet(
        self, planet_name: str, model_prediction: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        results = {"planet_name": planet_name, "status": "evaluated"}
        ...
        results["humidity"] = {
            ...
            "acceptable": humidity_relative_error
            < 1.0,  # 100% tolerance (humidity is highly variable)
        }

        # Overall assessment
        all_acceptable = (
            results["temperature"]["acceptable"]
            and results["pressure"]["acceptable"]
            and results["humidity"]["acceptable"]
        )
...
class PerformanceProfiler:
    ...
    def profile_model_inference(
        self, model, test_data: torch.Tensor, num_runs: int = 10
    ) -> Dict[str, Any]:
                ...
                cube = xr.Dataset(
                    {
                        ...
                        "q_H2O": (
                            ["lat", "lon", "pressure"],
                            (
                                pred_tensor[1].cpu().numpy()
                                if pred_tensor.size(0) > 1
                                else np.zeros_like(pred_tensor[0].cpu().numpy())
                            ),
                        ),
                        "albedo": (
                            ["lat", "lon"],
                            (
                                pred_tensor[2].cpu().numpy()[:, :, 0]
                                if pred_tensor.size(0) > 2
                                else np.ones((len(lat), len(lon))) * 0.3
                            ),
                        ),
                    },
        ...
        for planet_name in self.config.benchmark_planets:
            ...
            climate_metrics = {
                ...
                "mean_surface_pressure": (
                    float(prediction[0, 1].mean()) if prediction.size(1) > 1 else 1.0
                ),
                "global_mean_humidity": (
                    float(prediction[0, 2].mean()) if prediction.size(1) > 2 else 0.01
                ),
            }

            # Evaluate
            result = self.benchmark_evaluator.evaluate_benchmark_planet(
                planet_name, {"climate_metrics": climate_metrics}
            )
    ...
    def generate_summary_metrics(self) -> Dict[str, Any]:
        ...
        if physics_results:
            ...
            summary["physics_validation"] = {
                ...
                "energy_balance_pass_rate": sum(
                    1
                    for r in physics_results
                    if r.get("energy_balance", {}).get("energy_balance_valid", False)
                )
                / len(physics_results),
                "hydrostatic_pass_rate": sum(
                    1
                    for r in physics_results
                    if r.get("hydrostatic_equilibrium", {}).get("hydrostatic_valid", False)
                )
                / len(physics_results),
                "mass_conservation_pass_rate": sum(
                    1
                    for r in physics_results
                    if r.get("mass_conservation", {}).get("mass_conservation_valid", False)
                )
                / len(physics_results),
            }
        ...
        if benchmark_results:
            acceptable_count = sum(
                ...
                if r.get("overall_assessment", {}).get("all_acceptable", False)
            )
            avg_score = np.mean(
                [r.get("overall_assessment", {}).get("score", 0) for r in benchmark_results]
            )
            ...
            summary["benchmark_evaluation"] = {
                ...
                "temperature_accuracy": np.mean(
                    [
                        1 - r.get("temperature", {}).get("relative_error", 1)
                        for r in benchmark_results
                    ]
                ),
                "pressure_accuracy": np.mean(
                    [1 - r.get("pressure", {}).get("relative_error", 1) for r in benchmark_results]
                ),
            }
