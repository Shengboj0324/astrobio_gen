...
def demonstrate_priority_3_capabilities():
    ...
    for system_name, data in unknowability_examples.items():
        ...
        print(
            f"   Total Irreducible Uncertainty: {(data['emergence_uncertainty'] + data['fundamental_unknowability'] + data['temporal_uncertainty'])/3:.2f}"
        )
        print(f"   Description: {data['description']}")

    # Calculate overall unknowability trends
    avg_fundamental = np.mean(
        [data["fundamental_unknowability"] for data in unknowability_examples.values()]
    )
    avg_emergence = np.mean(
        [data["emergence_uncertainty"] for data in unknowability_examples.values()]
    )
    avg_temporal = np.mean(
        [data["temporal_uncertainty"] for data in unknowability_examples.values()]
    )
    ...
    strong_emergence_count = sum(
        1 for data in emergence_examples.values() if data["predictability_score"] < 0.2
    )
    downward_causation_count = sum(
        1 for data in emergence_examples.values() if data["downward_causation"]
    )
    ...
    avg_path_dependence = np.mean(
        [data["path_dependence_strength"] for data in path_dependence_examples.values()]
    )
    avg_historical_influence = np.mean(
        [data["historical_influence"] for data in path_dependence_examples.values()]
    )
    ...
    unknowable_systems = sum(
        1 for p in system_profiles.values() if p["fundamental_unknowability"] > 0.7
    )
    emergence_dominated = sum(
        1 for p in system_profiles.values() if p["emergence_indicators"] > 0.7
    )
