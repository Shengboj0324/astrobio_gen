...
class DataExpansionDemo:
    ...
    async def simulate_domain_processing(self, domain: str, sources: list):
        ...
        for i, source in enumerate(sources):
            ...
            success_probability = 0.95 if source.priority == 1 else 0.90 if source.priority == 2 else 0.85
        ...
        avg_quality = np.mean(domain_results['quality_scores']) if domain_results['quality_scores'] else 0.0
        
        logger.info(f"  ✅ {domain.upper()}: {domain_results['successful']}/{len(sources)} "
    ...
    def analyze_results(self, results: dict):
        ...
        for domain, stats in results['domain_statistics'].items():
            ...
            logger.info(f"  {domain.upper():>12}: {stats['successful']:>3}/{total_domain:>3} "
        ...
        if all_qualities:
            ...
            logger.info(f"  Sources ≥ 90% quality: {above_90}/{len(all_qualities)} "
                       f"({above_90/len(all_qualities)*100:.1f}%)")
            logger.info(f"  Sources ≥ 95% quality: {above_95}/{len(all_qualities)} "
                       f"({above_95/len(all_qualities)*100:.1f}%)")
            logger.info(f"  Sources ≥ 96% quality: {above_96}/{len(all_qualities)} "
        ...
        if analysis['overall_quality'] >= 0.964:
            ...
            for domain, stats in analysis['domain_stats'].items():
                ...
                if domain_quality < 0.90:
                    ...
                    recommendations.append(f"📊 {domain.upper()}: Good progress, minor improvements needed")
                else:
                    recommendations.append(f"✅ {domain.upper()}: Excellent quality achieved")
            
            # General recommendations
            if analysis['success_rate'] < 95:
                recommendations.append("🔧 Improve source accessibility and integration reliability")
        ...
        for domain, sources in self.expansion_system.data_sources.items():
            comprehensive_report['data_source_registry'][domain] = {
                ...
                'data_types': {
                    ...
                    'other': len([s for s in sources if s.data_type not in ['tabular', 'fits', 'netcdf']])
                },
...
async def run_comprehensive_demo():
    ...
    logger.info(f"🎯 Target (96.4%): {'ACHIEVED' if analysis['target_achieved'] else 'IN PROGRESS'}")
