...
class MockDataSystem:
    ...
    def __init__(self):
        self.domains = {
            "astrobiology": ["NASA Exoplanet Archive", "ESA Gaia", "TESS", "Kepler", "JWST Archive"],
...
def query_exoplanet_data(planet_name: Optional[str] = None, 
    ...
    known_planets = [
        {"name": "Kepler-452b", "star": "Kepler-452", "radius": 1.6, "period": 385, "hab_zone": True, "temp": 265},
        {"name": "TOI-715 b", "star": "TOI-715", "radius": 1.55, "period": 19.3, "hab_zone": True, "temp": 280},
        {"name": "TRAPPIST-1e", "star": "TRAPPIST-1", "radius": 0.92, "period": 6.1, "hab_zone": True, "temp": 251},
        {"name": "Proxima Centauri b", "star": "Proxima Centauri", "radius": 1.1, "period": 11.2, "hab_zone": True, "temp": 234},
        {"name": "K2-18b", "star": "K2-18", "radius": 2.6, "period": 33, "hab_zone": True, "temp": 279}
    ]
    ...
    return {
        ...
        "statistics": {
            ...
            "with_atmosphere_data": len([p for p in filtered_planets if p["atmospheric_data_available"]])
        },
...
def analyze_atmospheric_composition(composition: Dict[str, float], 
    ...
    normalized = {gas: ratio/total for gas, ratio in composition.items()} if total > 0 else composition
    ...
    for gas, props in biosig_gases.items():
        if normalized.get(gas, 0) > props["threshold"]:
            biosignatures.append({
                ...
                "detection_difficulty": "moderate" if normalized[gas] > props["threshold"] * 5 else "high"
            })
    ...
    return {
        ...
        "reference_comparison": {
            ...
            "key_differences": {k: abs(normalized.get(k, 0) - ref_comp.get(k, 0)) 
                             for k in all_gases if abs(normalized.get(k, 0) - ref_comp.get(k, 0)) > 0.01}
        },
        "observational_strategy": {
            "priority": "high" if len(biosignatures) > 1 else "medium",
            "recommended_instruments": ["NIRSpec", "MIRI"] if normalized.get("O2", 0) > 0.01 else ["NIRSpec"],
...
def search_scientific_database(query: str, 
    ...
    return {
        ...
        "search_summary": {
            "avg_relevance": round(sum(r["relevance_score"] for r in search_results) / len(search_results), 3) if search_results else 0,
...
class EnhancedChatDemo:
    ...
    async def run_comprehensive_demo(self):
        ...
        print(f"  • Metabolic network: {result1['metabolic_network']['nodes']} nodes, {result1['metabolic_network']['edges']} edges")
        ...
        print(f"  • Sample planets: {', '.join([p['name'] for p in result2['sample_planets'][:3]])}")
        ...
        print(f"✓ Analyzed atmosphere with {len(result3['potential_biosignatures'])} potential biosignatures")
        ...
        if result3['potential_biosignatures']:
            print(f"  • Key biosignatures: {', '.join([b['gas'] for b in result3['potential_biosignatures']])}")
        
        # Demo 4: Scientific Database Search
        print("\n📚 Demo 4: Scientific Database Search")
        print("-" * 40)
        result4 = search_scientific_database("exoplanet atmospheres JWST", domain="astrobiology", max_results=8)
    ...
    def show_enhancement_comparison(self):
        ...
        for orig, enh in zip(comparison["Original Chat System"], comparison["Enhanced Chat System"]):
            print(f"  {orig}")
            print(f"  ↓")
            print(f"  ✅ {enh}\n")
...
async def main():
    ...
    print(f"   🎯 Response Capabilities: {len(metrics['enhancement_metrics']['response_capabilities'])}")
    print(f"   💬 Conversation Features: {len(metrics['enhancement_metrics']['conversation_features'])}")
