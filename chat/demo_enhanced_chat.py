...
class EnhancedChatDemo:
    ...
    async def run_comprehensive_demo(self):
        ...
        print(f"Simulated {result1['planet']} with detectability score: {result1['detectability_score']}")
        ...
        print("\n📖 Demo 5: Research Summary Generation")
        print("-" * 40)
        result5 = generate_research_summary("biosignatures in exoplanet atmospheres", max_sources=15)
        ...
        print("\n🌈 Demo 8: Spectral Library Access")
        print("-" * 40)
        result8 = access_spectral_library(wavelength_range=(0.5, 5.0), spectral_type="atmospheric", resolution="high")
        ...
        print(f"Wavelength range: {result8['search_parameters']['wavelength_range_microns'][0]}-{result8['search_parameters']['wavelength_range_microns'][1]} μm")
        ...
        conversations = [
            {
                ...
                "tools_used": ["query_exoplanet_data", "calculate_habitability_metrics", "generate_research_summary"],
            ...
            },
            {
                ...
                "tools_used": ["access_spectral_library", "search_scientific_database", "generate_research_summary"],
        ...
        for tool_name, result in self.demo_results.items():
            if isinstance(result, dict):
                ...
                if "data_quality_score" in result:
                    ...
                    metrics["data_quality_scores"].append(result["research_overview"]["data_quality_avg"])
                    
        # Calculate averages
        if metrics["data_quality_scores"]:
            metrics["average_data_quality"] = sum(metrics["data_quality_scores"]) / len(metrics["data_quality_scores"])
        else:
