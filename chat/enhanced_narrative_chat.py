...
class PhilosophicalGuidanceEngine:
    ...
    def suggest_philosophical_framework(
        self, limit_types: List[QuantitativeLimitType]
    ) -> Dict[str, Any]:
    ...
    def _generate_integration_advice(self, frameworks: List[Dict]) -> str:
        ...
        base_advice = integration_templates.get(
            primary_framework, "Integrate multiple philosophical perspectives."
        )

        if len(frameworks) > 1:
            base_advice += (
                f" Also consider {frameworks[1]['framework']} to complement this approach."
            )
...
class EvolutionaryNarrativeBuilder:
    ...
    def construct_narrative(
        self, context: NarrativeContext, quantitative_results: Dict[str, Any]
    ) -> Dict[str, Any]:
            ...
            patterns["thresholds_detected"].append(
                {
                    ...
                    "value": (
                        float(disequilibrium) if hasattr(disequilibrium, "item") else disequilibrium
                    ),
    ...
    def _identify_narrative_gaps(
        self, patterns: Dict[str, Any], limit_types: List[QuantitativeLimitType]
    ) -> List[Dict[str, Any]]:
        ...
        gap_templates = {
            QuantitativeLimitType.EMERGENCE_THRESHOLD: {
                ...
                "questions": [
                    "Why does this particular organization create new behaviors?",
                    "What makes this transition qualitatively different?",
                ],
            },
            QuantitativeLimitType.TEMPORAL_CONTINGENCY: {
                ...
                "questions": [
                    "What made this pathway more likely than alternatives?",
                    "How did historical events constrain future possibilities?",
                ],
            },
            QuantitativeLimitType.PHILOSOPHICAL_BOUNDARY: {
                ...
                "questions": [
                    "What distinguishes living from non-living processes?",
                    "How do we define biological agency?",
                ],
            },
            QuantitativeLimitType.COMPLEXITY_OVERFLOW: {
                ...
                "questions": [
                    "How does the system maintain stability amid complexity?",
                    "What principles govern large-scale organization?",
                ],
            },
        }
    ...
    def _build_coherent_narrative(
        ...
        template_info = self.narrative_templates.get(
            phase, self.narrative_templates["complex_life"]
        )
    ...
    def _describe_quantitative_foundation(self, patterns: Dict[str, Any]) -> str:
        ...
        for trend in patterns.get("temporal_trends", []):
            descriptions.append(
                f"{trend['variable']} shows a {trend['trend']} pattern with {trend['confidence']} confidence"
            )

        for threshold in patterns.get("thresholds_detected", []):
            descriptions.append(
                f"Detected {threshold['threshold_type']} threshold with significance for {threshold['significance']}"
            )
    ...
    def _interpret_qualitative_meaning(
        self, patterns: Dict[str, Any], gaps: List[Dict[str, Any]]
    ) -> str:
        ...
        for gap in gaps:
            if gap["gap_type"] == "emergence_explanation":
                interpretations.append(
                    "The emergence of new properties suggests qualitative transitions that transcend quantitative thresholds"
                )
            elif gap["gap_type"] == "historical_contingency":
                interpretations.append(
                    "The specific evolutionary pathway reflects historical contingency and path-dependent processes"
                )
            elif gap["gap_type"] == "conceptual_boundary":
                interpretations.append(
                    "Fundamental conceptual questions about the nature of life emerge"
                )
    ...
    def _synthesize_quantitative_qualitative(
        self, patterns: Dict[str, Any], gaps: List[Dict[str, Any]], context: NarrativeContext
    ) -> str:
    ...
    def _acknowledge_uncertainties(
        self, gaps: List[Dict[str, Any]], template_info: Dict[str, Any]
    ) -> str:
        """Acknowledge uncertainties and limitations"""
        uncertainty_factors = template_info.get(
            "uncertainty_factors", ["temporal_processes", "complex_interactions"]
        )

        acknowledgment = (
            f"Significant uncertainties remain regarding {', '.join(uncertainty_factors[:2])}."
        )
    ...
    def _explore_broader_implications(
        self, context: NarrativeContext, patterns: Dict[str, Any]
    ) -> str:
    ...
    def _compose_narrative_text(
        self, components: Dict[str, str], template_info: Dict[str, Any], context: NarrativeContext
    ) -> str:
    ...
    def _extract_philosophical_insights(
        self, narrative: Dict[str, Any], context: NarrativeContext
    ) -> List[str]:
        ...
        if context.evolutionary_timescale and context.evolutionary_timescale > 3.0:
            insights.append(
                "Deep time perspective reveals the contingent nature of evolutionary outcomes"
            )

        if QuantitativeLimitType.EMERGENCE_THRESHOLD in context.limit_types_encountered:
            insights.append(
                "Emergence demonstrates that wholes possess properties not predictable from parts"
            )

        if QuantitativeLimitType.TEMPORAL_CONTINGENCY in context.limit_types_encountered:
            insights.append(
                "Historical contingency shows that current life forms represent one of many possible outcomes"
            )

        insights.append(
            "The integration of quantitative and qualitative approaches is essential for understanding life"
        )

        return insights

    def _suggest_narrative_research_directions(
        self, gaps: List[Dict[str, Any]], context: NarrativeContext
    ) -> List[str]:
        ...
        for gap in gaps:
            if gap["gap_type"] == "emergence_explanation":
                directions.append(
                    "Investigate mechanisms of emergence through multi-level analysis"
                )
            elif gap["gap_type"] == "historical_contingency":
                directions.append(
                    "Explore alternative evolutionary scenarios through comparative planetology"
                )
            elif gap["gap_type"] == "conceptual_boundary":
                directions.append("Engage in interdisciplinary dialogue on the definition of life")

        directions.append(
            "Develop methodologies that integrate quantitative and narrative approaches"
        )
...
class EnhancedNarrativeChat:
    ...
    def process_research_query(
        self, user_input: str, current_results: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        """
    ...
    def _analyze_user_input(
        self, user_input: str, current_results: Optional[Dict[str, Any]]
    ) -> Dict[str, Any]:
                ...
                for i, word in enumerate(words):
                    if "billion" in word and i > 0:
                        try:
                            ...
                            analysis["evolutionary_timescale"] = (
                                3.5  # Default to approximate life origin
                            )
    ...
    def _identify_quantitative_limits(
        self, analysis: Dict[str, Any], current_results: Optional[Dict[str, Any]]
    ...
    def _handle_quantitative_phase(
        self, context: NarrativeContext, user_input: str
    ) -> Dict[str, Any]:
    ...
    def _handle_boundary_identification(
        self, context: NarrativeContext, user_input: str
    ) -> Dict[str, Any]:
        ...
        detected_limits = [
            limit_descriptions.get(limit, str(limit)) for limit in context.limit_types_encountered
        ]
        ...
        return {
            ...
            "philosophical_frameworks_suggested": self.philosophical_engine.suggest_philosophical_framework(
                context.limit_types_encountered
            ),
            "next_steps": [
    ...
    def _handle_paradigm_transition(
        self, context: NarrativeContext, user_input: str
    ) -> Dict[str, Any]:
        """Handle paradigm transition guidance"""
        philosophical_guidance = self.philosophical_engine.suggest_philosophical_framework(
            context.limit_types_encountered
        )
    ...
    def _handle_qualitative_exploration(
        self, context: NarrativeContext, user_input: str
    ) -> Dict[str, Any]:
    ...
    def _handle_narrative_construction(
        self, context: NarrativeContext, user_input: str
    ) -> Dict[str, Any]:
        """Handle evolutionary narrative construction"""
        # Use the narrative builder to construct evolutionary story
        narrative_result = self.narrative_builder.construct_narrative(
            context, context.quantitative_findings
        )
    ...
    def _handle_philosophical_integration(
        self, context: NarrativeContext, user_input: str
    ) -> Dict[str, Any]:
    ...
    def _log_conversation(
        self, user_input: str, response: Dict[str, Any], context: NarrativeContext
    ):
    ...
    def get_conversation_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "current_phase": (
                self.current_context.current_phase.value if self.current_context else "unknown"
            ),
            "research_question": (
                self.current_context.research_question if self.current_context else "unknown"
            ),
            "limits_encountered": (
                [lt.value for lt in self.current_context.limit_types_encountered]
