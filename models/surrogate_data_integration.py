...
try:
    ...
    from pipeline.generate_spectrum_psg import PSGInterface, get_spectrum
...
class SurrogateDataManager:
    ...
    def _initialize_enterprise_systems(self):
        ...
        try:
            if URL_SYSTEM_AVAILABLE:
                logger.info(
                    "[NET] Initializing enterprise data acquisition for surrogate models..."
                )
                ...
                logger.warning(
                    "[WARN] Enterprise URL system not available, using fallback data sources"
                )

        except Exception as e:
            logger.error(f"[FAIL] Failed to initialize enterprise data systems: {e}")

    async def acquire_training_data(
        self, n_samples: int = 1000, data_types: List[str] = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"[TARGET] Training data acquisition complete. Average quality: {np.mean(quality_scores):.3f}"
            )
    ...
    async def _acquire_planetary_data(self, n_samples: int) -> List[PlanetaryData]:
        ...
        try:
            if self.url_system:
                ...
                nasa_url = await self.url_system.get_url(test_url, priority=DataPriority.HIGH)
    ...
    def preprocess_for_surrogate(
        self, training_data: Dict[str, Any], target_mode: str = "scalar"
        ...
        try:
            ...
            if target_mode == "scalar":
                ...
                if targets:
                    processed_data["scalar_targets"] = torch.tensor(targets, dtype=torch.float32)
                ...
                for climate_data in training_data.get("climate_profiles", []):
                    ...
                    temp_cube = torch.tensor(
                        climate_data.get("temperature", np.random.normal(250, 50, (10, 16, 32))),
                        dtype=torch.float32,
                    )
                ...
                for spectral_data in training_data.get("spectral_data", []):
                    spectrum = torch.tensor(
                        spectral_data.get("spectrum", np.random.normal(1, 0.1, 1000)),
                        dtype=torch.float32,
                    )
            ...
            logger.info(
                f"[DATA] Processed shapes: {[(k, v.shape) for k, v in processed_data.items() if isinstance(v, torch.Tensor)]}"
            )
    ...
    def _compute_habitability(self, planet_data: PlanetaryData) -> float:
        ...
        try:
            ...
            temp_score = (
                1.0
                if habitable_temp_range[0] <= equilibrium_temp <= habitable_temp_range[1]
                else 0.0
            )
