...
class PhysicsRegularizer(nn.Module):
    ...
    def forward(
        self, predictions: torch.Tensor, variable_names: List[str]
        ...
        if self.constraints.energy_conservation and "temp" in var_idx:
            ...
            losses["temperature_gradient"] = torch.mean(
                torch.clamp(torch.abs(dt_dz) - 0.1, min=0) ** 2
            )
        ...
        if self.constraints.hydrostatic_balance and "pressure" in var_idx and "temp" in var_idx:
        ...
        if "humidity" in var_idx:
            ...
            losses["humidity_bounds"] = torch.mean(
                torch.clamp(humidity - 1.0, min=0) ** 2 + torch.clamp(-humidity, min=0) ** 2
            )
...
class CubeUNet(pl.LightningModule):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        logger.info(
            f"Initialized CubeUNet with {n_input_vars} input vars, "
        ...
        for i in range(self.depth):
            if i == 0:
                ...
                self.downsample_blocks.append(
                    DownSample3D(in_channels, features, dropout=self.dropout)
                )
                encoder_features.append(features)
