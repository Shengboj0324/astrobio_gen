...
class ScientificTextEncoder(nn.Module):
    ...
    def __init__(self, config: ClimateGenerationConfig):
        ...
        try:
            self.scientific_tokenizer = AutoTokenizer.from_pretrained(config.scientific_encoder_name)
    ...
    def _build_climate_vocabulary(self) -> Dict[str, int]:
        ...
        climate_terms = {
            ...
            'water_vapor', 'carbon_dioxide', 'methane', 'oxygen', 'nitrogen',
            'hydrogen', 'helium', 'argon', 'ozone', 'aerosols',
            
            # Physical processes
            'convection', 'radiation', 'circulation', 'turbulence', 'condensation',
            'evaporation', 'precipitation', 'greenhouse_effect', 'albedo',
            
            # Temperature concepts
            'temperature', 'thermal', 'heating', 'cooling', 'isothermal',
            'adiabatic', 'inversion', 'gradient', 'stratosphere', 'troposphere',
            
            # Pressure and dynamics
            'pressure', 'winds', 'circulation', 'vorticity', 'jet_stream',
            'coriolis', 'geostrophic', 'cyclone', 'anticyclone',
            
            # Planetary concepts
            'tidally_locked', 'day_night', 'terminator', 'polar', 'equatorial',
            'seasonal', 'orbital', 'rotation', 'synchronous',
            
            # Exoplanet types
            'super_earth', 'hot_jupiter', 'mini_neptune', 'terrestrial',
            'ocean_world', 'desert_planet', 'ice_world', 'lava_planet'
        }
    ...
    def _extract_climate_features(self, text_inputs: List[str]) -> torch.Tensor:
        ...
        climate_embeddings = self.climate_embeddings(torch.arange(len(self.climate_vocab), device=device))
...
class Physics3DUNet(nn.Module):
    ...
    def forward(self, 
                ...
                planet_embeds: Optional[torch.Tensor] = None) -> torch.Tensor:
...
class PhysicsConstraintLayer(nn.Module):
    ...
    def _enforce_hydrostatic_balance(self, temp: torch.Tensor, pressure: torch.Tensor) -> torch.Tensor:
    ...
    def _enforce_mass_conservation(self, u_wind: torch.Tensor, v_wind: torch.Tensor) -> torch.Tensor:
...
class MultimodalClimateGenerator(nn.Module):
    ...
    def __init__(self, config: Optional[ClimateGenerationConfig] = None):
        ...
        if self.config.uncertainty_estimation:
            self.uncertainty_head = nn.Conv3d(self.config.num_variables, self.config.num_variables, 1)
    ...
    @torch.no_grad()
    def generate(self, request: GenerationRequest, num_inference_steps: int = 50) -> Dict[str, torch.Tensor]:
        ...
        if request.planet_parameters:
            planet_tensor = torch.tensor([
                list(request.planet_parameters.values())
        ...
        for t in self.scheduler.timesteps:
            ...
            if request.guidance_scale > 1.0:
                ...
                # Apply guidance
                noise_pred = uncond_noise_pred + request.guidance_scale * (noise_pred - uncond_noise_pred)
...
def create_climate_generator(spatial_resolution: Tuple[int, int, int] = (64, 64, 32),
                           enable_physics: bool = True) -> MultimodalClimateGenerator:
...
async def demonstrate_climate_generation():
    ...
    result3 = generator.generate_from_text(
        "Rocky planet with thin CO2 atmosphere and dust storms",
        random_seed=42
    )
