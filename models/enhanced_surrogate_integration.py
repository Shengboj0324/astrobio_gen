...
class CrossAttentionFusion(nn.Module):
    ...
    def forward(self, cnn_features: torch.Tensor, 
        ...
        # Cross-attention
        cnn_attended, _ = self.cnn_to_transformer(
            cnn_proj, transformer_proj, transformer_proj
        )
        
        transformer_attended, _ = self.transformer_to_cnn(
            transformer_proj, cnn_proj, cnn_proj
        )
...
class EnhancedSurrogateIntegration(pl.LightningModule):
    ...
    def _build_fusion_layer(self):
        ...
        if self.multimodal_config.fusion_strategy == "cross_attention":
            ...
            cnn_dim = self.datacube_model.base_features if hasattr(self.datacube_model, 'base_features') else 256
            transformer_dim = self.transformer_model.d_model if hasattr(self.transformer_model, 'd_model') else 256
        ...
        # Output layer
        self.output_layer = nn.Linear(self.multimodal_config.hidden_dim, 
                                     self.datacube_config.get('n_output_vars', 5))
    ...
    def _build_dynamic_selection(self):
        ...
        self.dynamic_selector = DynamicModelSelection(
            input_dim=input_dim,
            num_models=3,  # Basic, Enhanced, Full models
            hidden_dim=128
        )
    ...
    def training_step(self, batch: Dict[str, torch.Tensor], batch_idx: int) -> torch.Tensor:
        ...
        if self.use_uncertainty and 'uncertainty' in outputs:
            ...
            nll_loss = 0.5 * torch.log(2 * math.pi * uncertainty) + \
        ...
        # Total loss
        total_loss = primary_loss + 0.1 * uncertainty_loss + 0.05 * physics_loss + 0.02 * distillation_loss
    ...
    def get_integration_complexity(self) -> Dict[str, Any]:
        ...
        if hasattr(self, 'transformer_model'):
            component_params['transformer'] = sum(p.numel() for p in self.transformer_model.parameters())
