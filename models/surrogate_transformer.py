...
class PhysicsConstraintLayer(nn.Module):
    ...
    def compute_mass_balance_constraint(self, predictions: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        mass_conservation = F.mse_loss(composition.sum(dim=-1), torch.ones_like(composition.sum(dim=-1)))
...
class SurrogateTransformer(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.register_parameter('physics_weights', 
                              nn.Parameter(torch.tensor([1.0, 1.0, 0.1])))
    ...
    def _build_output_heads(self) -> nn.ModuleDict:
        ...
        if self.mode == "scalar":
            ...
            heads['spectrum'] = nn.Sequential(
                ...
                nn.ReLU(),
                nn.Linear(2048, 10000)  # 10k wavelength bins
            )
    ...
    def forward(self, x: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        if self.mode == "datacube":
            if 'temperature_field' in predictions:
                predictions['temperature_field'] = predictions['temperature_field'].view(-1, 64, 32, 20)
    ...
    def compute_physics_loss(self, outputs: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        physics_loss = (weights[0] * radiative_loss + 
                       weights[1] * mass_balance_loss)
        
        return physics_loss
    
    def compute_total_loss(self, outputs: Dict[str, torch.Tensor], targets: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        for key in targets:
            if key in outputs:
                if 'field' in key:  # 3D fields
                    ...
                    losses[f'{key}_loss'] = F.binary_cross_entropy_with_logits(outputs[key], targets[key])
                else:
                    losses[f'{key}_loss'] = F.mse_loss(outputs[key], targets[key])
