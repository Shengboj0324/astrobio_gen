...
from torch_geometric.nn import GATConv, GCNConv, SAGEConv, GINConv, global_mean_pool, global_max_pool
...
class AdvancedGraphNeuralNetwork(nn.Module):
    ...
    def __init__(self, 
                 config: GraphConfig,
                 task_type: str = "metabolic_network",
                 output_dim: int = 64):
        super().__init__()
        self.config = config
    ...
    def forward(self, data: Data) -> Dict[str, torch.Tensor]:
        ...
        for i, gat_layer in enumerate(self.gat_layers):
            ...
            if (self.config.use_hierarchical_pooling and 
                i < len(self.pool_layers) and 
                i % 2 == 1):  # Pool every other layer
...
def create_graph_neural_network(task_type: str = "metabolic_network", 
                               config: Optional[GraphConfig] = None) -> AdvancedGraphNeuralNetwork:
