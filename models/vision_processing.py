...
try:
    ...
    CV2_AVAILABLE = False
    warnings.warn(
        "OpenCV and PIL not available. Install with: pip install opencv-python pillow albumentations"
    )
...
class ImagePreprocessor:
    ...
    def preprocess_image(
        self,
        image_input: Union[str, bytes, np.ndarray, "PIL.Image.Image"],
        ...
        try:
            ...
            elif CV2_AVAILABLE and hasattr(
                image_input, "save"
            ):  # PIL Image check (has save method)
                image = image_input
            else:
...
class AdvancedImageAnalyzer:
    ...
    async def analyze_image(
        self, image_input: Any, analysis_type: str = "comprehensive"
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            device = (
                next(iter(self.models.values())).parameters().__next__().device
                if self.models
                else "cpu"
            )
            ...
            return {"error": str(e), "processing_time": time.time() - start_time, "success": False}
    ...
    async def _clip_analysis(self, image_tensor: torch.Tensor) -> Dict[str, Any]:
        ...
        try:
            ...
            text_inputs = self.clip_processor(
                text=scientific_prompts, return_tensors="pt", padding=True
            )
        ...
        try:
            ...
            if SCIENTIFIC_PROCESSING_AVAILABLE:
                ...
                glcm = feature.graycomatrix(
                    gray_uint8, [1], [0], levels=256, symmetric=True, normed=True
                )
...
class VideoProcessor:
    ...
    async def process_video(
        self, video_input: Union[str, bytes], analysis_type: str = "comprehensive"
    ) -> Dict[str, Any]:
        """
        ...
        try:
            if DECORD_AVAILABLE and isinstance(video_input, str):
                ...
                frame_indices = np.linspace(
                    0, total_frames - 1, self.config.video_frames, dtype=int
                )
    ...
    async def _analyze_frames(self, frames: List[np.ndarray]) -> Dict[str, Any]:
        ...
        return {"individual_frames": frame_results, "num_frames_analyzed": len(frame_results)}
