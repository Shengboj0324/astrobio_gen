...
class CrossModalFusionLayer(nn.Module):
    ...
    def __init__(self, config: FusionConfig):
        ...
        if config.use_adaptive_weights:
            self.fusion_weights = nn.Parameter(torch.ones(3, 3) / 3)  # 3x3 for text, vision, scientific
        else:
    ...
    def forward(self, 
                ...
                attention_masks: Optional[Dict[str, torch.Tensor]] = None) -> Dict[str, torch.Tensor]:
        """
        ...
        if scientific_features is not None:
            projected_features['scientific'] = self.modality_projections['scientific'](scientific_features)
        ...
        for modality, features in projected_features.items():
            if modality == 'text' and features is not None:
                ...
                refined_features['vision'], _ = self.vision_self_attention(features, features, features)
            elif modality == 'scientific' and features is not None:
                refined_features['scientific'], _ = self.scientific_self_attention(features, features, features)
        ...
        if 'text' in refined_features and 'vision' in refined_features:
            ...
            fused_features['text'] = fused_features['text'] + self.fusion_weights[0, 1] * text_from_vision
            ...
            fused_features['vision'] = fused_features['vision'] + self.fusion_weights[1, 0] * vision_from_text
        ...
        if 'text' in refined_features and 'scientific' in refined_features:
            ...
            fused_features['text'] = fused_features['text'] + self.fusion_weights[0, 2] * text_from_scientific
            ...
            fused_features['scientific'] = fused_features['scientific'] + self.fusion_weights[2, 0] * scientific_from_text
        ...
        if 'vision' in refined_features and 'scientific' in refined_features:
            ...
            fused_features['vision'] = fused_features['vision'] + self.fusion_weights[1, 2] * vision_from_scientific
            ...
            fused_features['scientific'] = fused_features['scientific'] + self.fusion_weights[2, 1] * scientific_from_vision
...
class AdaptiveFusionStrategy(nn.Module):
    ...
    def __init__(self, config: FusionConfig):
        ...
        self.hierarchical_fusion = nn.ModuleList([
            ...
            for _ in range(2)  # Two levels
        ])
    
    def forward(self, 
                text_features: Optional[torch.Tensor] = None,
                vision_features: Optional[torch.Tensor] = None,
                scientific_features: Optional[torch.Tensor] = None) -> torch.Tensor:
        """
        ...
        weighted_result = torch.sum(
            results * strategy_weights.unsqueeze(-1), 
            dim=1
        )
...
class CrossModalFusionNetwork(nn.Module):
    ...
    def __init__(self, config: FusionConfig = None):
        ...
        self.fusion_layers = nn.ModuleList([
            CrossModalFusionLayer(self.config)
...
class ScientificDataIntegrator(nn.Module):
    ...
    def __init__(self, config: FusionConfig):
        ...
        self.feature_harmonizer = nn.ModuleDict({
            ...
            'enhanced_surrogate': nn.Sequential(
                nn.Linear(512, config.hidden_dim),  # Assuming 512 from EnhancedSurrogateIntegration
                nn.GELU(),
                nn.LayerNorm(config.hidden_dim)
            )
        })
    ...
    def integrate_scientific_data(self, 
                                scientific_data: Dict[str, torch.Tensor]) -> torch.Tensor:
        """
