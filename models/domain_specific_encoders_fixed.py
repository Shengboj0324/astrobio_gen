...
class FixedDomainPhysicsConstraintLayer(nn.Module):
...
class MultiModalEncoder(nn.Module):
    ...
    def __init__(self, config: EncoderConfig):
        ...
        if config.use_physics_constraints:
            self.physics_layer = FixedDomainPhysicsConstraintLayer(config.latent_dim)
        else:
...
if __name__ == "__main__":
    ...
    def test_multimodal_encoder():
        logger.info("[TEST] Testing Multi-Modal Encoder Architecture (Fixed)")
