...
class TelescopeController:
    ...
    async def check_target_visibility(self, target: ExperimentTarget, 
    ...
    async def schedule_observation(self, target: ExperimentTarget, 
                                 duration: timedelta,
                                 observation_time: datetime) -> Dict[str, Any]:
    ...
    async def execute_observation(self, booking_id: str, 
...
class LaboratoryController:
    ...
    async def design_experimental_matrix(self, parameters: Dict[str, Any],
    ...
    def _generate_d_optimal_design(self, param_ranges: Dict[str, List], 
                                 n_experiments: int) -> List[Dict[str, float]]:
    ...
    async def execute_experiment(self, experiment_params: Dict[str, Any],
...
class AdvancedExperimentOrchestrator:
    ...
    def _initialize_optimizer(self) -> GaussianProcessRegressor:
        ...
        optimizer = GaussianProcessRegressor(kernel=kernel, alpha=1e-6, 
                                           normalize_y=True, n_restarts_optimizer=5)
        return optimizer
    
    async def design_observational_campaign(self, targets: List[str],
                                          objectives: List[str],
    ...
    async def execute_laboratory_experiments(self, experiment_type: str,
                                           parameters: Dict[str, Any],
                                           constraints: Dict[str, Any]) -> Dict[str, Any]:
        """Execute autonomous laboratory experiments"""
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                failed_experiments.append({
                    'experiment_id': design_matrix.iloc[i]['experiment_id'],
                    'error': str(result)
                })
            else:
        ...
        logger.info(f"🧪 Laboratory campaign completed: {len(successful_experiments)}/{len(design_matrix)} successful")
    ...
    async def _optimize_observation_schedule(self, targets: List[ExperimentTarget],
                                           total_time: float,
                                           telescopes: List[str],
            ...
            for j, telescope in enumerate(telescopes):
                ...
                if time_allocated > 0.1:  # Minimum observation time
                    observation = {
                        ...
                        'estimated_start': datetime.now() + timedelta(days=j),  # Stagger start times
                        'objectives': objectives
                    }
        ...
        logger.info(f"🎯 Schedule optimized: {len(schedule)} observations, {optimization_results['time_utilization']:.1%} time utilization")
        
        return optimization_results
    
    def _calculate_capability_match(self, target: ExperimentTarget,
    ...
    def _estimate_data_products(self, targets: List[ExperimentTarget],
                              objectives: List[str]) -> Dict[str, Any]:
    ...
    def _select_optimal_laboratory(self, experiment_type: str,
    ...
    def _analyze_experiment_results(self, results: List[Dict[str, Any]],
        ...
        if experiment_type == 'biosignature_synthesis':
            ...
            analysis['summary_statistics'] = {
                ...
                'reaction_products_avg': np.mean([r['measurements']['reaction_products'] for r in results])
            }
        ...
        for result in results:
            ...
            if qc:
                ...
                quality_factors = [v for v in qc.values() if isinstance(v, (int, float)) and 0 <= v <= 1]
                if quality_factors:
        ...
        if quality_scores:
            analysis['quality_assessment'] = {
                ...
                'high_quality_fraction': len([q for q in quality_scores if q > 0.8]) / len(quality_scores)
            }
        
        return analysis
    
    async def _optimize_experimental_conditions(self, results: List[Dict[str, Any]],
                                              parameters: Dict[str, Any],
                                              experiment_type: str) -> Dict[str, Any]:
        ...
        try:
            ...
            optimization_results = {
                'status': 'success',
                'optimal_conditions': {
                    param_names[i]: best_conditions[i] 
    ...
    def _generate_experiment_recommendations(self, analysis_results: Dict[str, Any]) -> List[str]:
        ...
        if quality_assessment:
            ...
            if mean_quality < 0.7:
                recommendations.append("Improve experimental quality: mean quality score is below threshold")
            
            high_quality_fraction = quality_assessment.get('high_quality_fraction', 0.5)
            if high_quality_fraction < 0.6:
                recommendations.append("Increase consistency: less than 60% of experiments achieved high quality")
            ...
            for metric, value in summary_stats.items():
                if 'std' in metric and isinstance(value, (int, float)):
                    if value > 10:  # High variability
                        recommendations.append(f"Reduce variability in {metric.replace('std_', '')}")
...
def create_experiment_orchestrator(config_path: Optional[str] = None) -> AdvancedExperimentOrchestrator:
...
async def demonstrate_advanced_experiment_orchestration():
    ...
    logger.info(f"   Success rate: {lab_campaign['successful_experiments']}/{lab_campaign['total_experiments']}")
    
    # Demonstration 3: System Status
    system_status = await orchestrator.get_system_status()
    logger.info(f"📊 System status: {len(system_status['telescopes'])} telescopes, {len(system_status['laboratories'])} labs")
    ...
    demo_results = {
        ...
        'demonstration_summary': {
            'total_experiments_designed': observational_campaign.get('expected_data_products', {}).get('spectra', 0) + lab_campaign['total_experiments'],
