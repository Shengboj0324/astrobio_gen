...
class Priority2Demonstration:
    ...
    def demonstrate_quantitative_limit_recognition(self) -> Dict[str, Any]:
        ...
        research_scenarios = [
            ...
            },
            {
                ...
                "current_results": {
                    "evolution_predictability": 0.2,
                    "historical_factors": "high_influence",
                },
            },
        ]
        ...
        for i, scenario in enumerate(research_scenarios):
            ...
            response = self.narrative_chat.process_research_query(
                scenario["user_input"], scenario["current_results"]
            )
        ...
        # Overall accuracy
        overall_accuracy = sum(r["recognition_accuracy"] for r in limit_recognition_results) / len(
            limit_recognition_results
        )
        ...
        return {
            ...
            "limit_types_covered": list(
                set([l.value for s in research_scenarios for l in s["expected_limits"]])
            ),
        }
    ...
    def demonstrate_philosophical_guidance(self) -> Dict[str, Any]:
        ...
        for i, test_case in enumerate(philosophical_test_cases):
            ...
            test_result = {
                ...
                "framework_relevance_scores": {
                    f["framework"]: f["relevance_score"] for f in recommended_frameworks
                },
                ...
                "guidance_quality": self._assess_philosophical_guidance_quality(
                    recommended_frameworks, test_case["expected_frameworks"]
                ),
            }
        ...
        overall_guidance_quality = sum(r["guidance_quality"] for r in guidance_results) / len(
            guidance_results
        )
        ...
        for i, scenario in enumerate(transition_scenarios):
            ...
            scenario_result = {
                ...
                "transition_quality": self._assess_transition_quality(
                    response, scenario["expected_transition"]
                ),
            }
            ...
            print(
                f"   Transition: {transition_guidance.get('from', '')} -> {transition_guidance.get('to', '')}"
            )
            print(f"   Quality: {scenario_result['transition_quality']:.2f}")

        average_transition_quality = sum(r["transition_quality"] for r in transition_results) / len(
            transition_results
        )
    ...
    def demonstrate_narrative_construction(self) -> Dict[str, Any]:
        ...
        for i, scenario in enumerate(narrative_scenarios):
            ...
            response = self.narrative_chat.process_research_query(
                scenario["user_input"], scenario["quantitative_data"]
            )
            ...
            scenario_result = {
                ...
                "integration_quality": narrative_framework.get("constructed_narrative", {}).get(
                    "integration_quality", 0
                ),
                "narrative_coherence": self._assess_narrative_coherence(
                    narrative_framework, scenario
                ),
            }
        ...
        average_narrative_quality = sum(r["narrative_coherence"] for r in narrative_results) / len(
            narrative_results
        )
        ...
        return {
            ...
            "evolutionary_periods_covered": [
                f"{s['evolutionary_timescale']} Gya" for s in narrative_scenarios
            ],
    ...
    def demonstrate_research_conversation_flow(self) -> Dict[str, Any]:
        ...
        for i, exchange in enumerate(conversation_flow):
            ...
            exchange_result = {
                ...
                "detected_phase": response.get("phase"),
                "phase_accuracy": (
                    1.0 if response.get("phase") == exchange["expected_phase"] else 0.5
                ),
                "guidance_provided": response.get("guidance", ""),
        ...
        conversation_progression_quality = sum(
            r["phase_accuracy"] for r in conversation_results
    ...
    def _assess_limit_recognition_accuracy(
        self, detected: List[QuantitativeLimitType], expected: List[QuantitativeLimitType]
    ) -> float:
        ...
        detected_values = [
            limit.value if hasattr(limit, "value") else str(limit) for limit in detected
        ]
    ...
    def _assess_philosophical_guidance_quality(
        self, recommended: List[Dict], expected: List[str]
    ) -> float:
    ...
    def _assess_transition_quality(
        self, response: Dict[str, Any], expected_transition: str
    ) -> float:
    ...
    def _assess_narrative_coherence(
        self, narrative_framework: Dict[str, Any], scenario: Dict[str, Any]
    ) -> float:
        ...
        coherence_factors = [
            ...
            narrative_framework.get("constructed_narrative", {}).get("integration_quality", 0)
            > 0.3,
        ]
    ...
    def save_demonstration_results(self):
        ...
        output_file = (
            ...
            / f"priority_2_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
    ...
    def create_summary_report(self):
        ...
        summary_file = (
            Path(self.demo_config["output_dir"])
            / f"priority_2_summary_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
        )
        ...
        with open(summary_file, "w") as f:
            ...
            f.write(
                "Successfully implemented enhanced narrative chat system that builds on Priority 1\n"
            )
            f.write(
                "evolutionary process modeling to help researchers navigate quantitative-qualitative boundaries.\n\n"
            )

            f.write("## Demonstrated Capabilities\n\n")
            f.write("### 1. Quantitative Limit Recognition\n")
            f.write(
                f"- Recognition accuracy: {self.results['quantitative_limit_recognition']['overall_accuracy']:.2f}\n"
            )
            f.write(
                f"- Scenarios tested: {self.results['quantitative_limit_recognition']['scenarios_tested']}\n"
            )
            f.write(
                f"- Limit types covered: {len(self.results['quantitative_limit_recognition']['limit_types_covered'])}\n\n"
            )

            f.write("### 2. Philosophical Guidance\n")
            f.write(
                f"- Guidance quality: {self.results['philosophical_guidance']['overall_quality']:.2f}\n"
            )
            f.write(
                f"- Frameworks tested: {len(self.results['philosophical_guidance']['frameworks_tested'])}\n"
            )
            f.write(f"- Test cases: {self.results['philosophical_guidance']['test_cases']}\n\n")

            f.write("### 3. Paradigm Transitions\n")
            f.write(
                f"- Transition quality: {self.results['paradigm_transitions']['average_quality']:.2f}\n"
            )
            f.write(
                f"- Paradigm shifts: {len(self.results['paradigm_transitions']['paradigm_shifts_demonstrated'])}\n\n"
            )

            f.write("### 4. Narrative Construction\n")
            f.write(
                f"- Narrative quality: {self.results['narrative_construction']['average_quality']:.2f}\n"
            )
            f.write(
                f"- Evolutionary periods: {len(self.results['narrative_construction']['evolutionary_periods_covered'])}\n\n"
            )

            f.write("### 5. Research Conversation Flow\n")
            f.write(
                f"- Progression quality: {self.results['research_conversation_flow']['progression_quality']:.2f}\n"
            )
            f.write(
                f"- Phases demonstrated: {len(self.results['research_conversation_flow']['phases_demonstrated'])}\n\n"
            )
            ...
            f.write(
                f"- Research scenarios tested: {self.results['demonstration_overview']['configuration']['research_scenarios']}\n"
            )
...
def main():
    ...
    print(
        f"Limit Recognition: {results['quantitative_limit_recognition']['overall_accuracy']:.2f} accuracy"
    )
    print(
        f"Philosophical Guidance: {results['philosophical_guidance']['overall_quality']:.2f} quality"
    )
    print(f"Paradigm Transitions: {results['paradigm_transitions']['average_quality']:.2f} quality")
    print(
        f"Narrative Construction: {results['narrative_construction']['average_quality']:.2f} quality"
    )
    print(
        f"Conversation Flow: {results['research_conversation_flow']['progression_quality']:.2f} quality"
    )
