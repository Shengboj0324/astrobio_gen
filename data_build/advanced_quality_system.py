...
class ValidityRule(QualityRule):
    ...
    def evaluate(self, data: Any) -> Tuple[bool, List[QualityIssue]]:
        ...
        if isinstance(data, pd.DataFrame):
                ...
                if field in data.columns:
                    ...
                    if "type" in constraints:
                        ...
                        if expected_type == "numeric":
                            ...
                            violations = (
                                ~pd.to_datetime(field_data, errors="coerce").notna()
                            ).sum()
                        else:
                            violations = 0
            ...
            validity_ratio = (
                1.0 - (constraint_violations / total_constraints) if total_constraints > 0 else 1.0
            )
                ...
                if field in data.columns:
                    ...
                    if len(field_data) > 10:
                        if self.method == "isolation_forest":
                            ...
                            outliers = field_data[
                                (field_data < lower_bound) | (field_data > upper_bound)
                            ]
...
class QualityRuleEngine:
    ...
    def __init__(self):
        self.rules: Dict[str, Dict[DataType, List[QualityRule]]] = defaultdict(
    ...
    def _register_default_rules(self):
        ...
        self.add_rule(
            DataType.KEGG_PATHWAY, ValidityRule({"reaction_count": {"min": 0, "type": "numeric"}})
        )
        ...
        self.add_rule(
            DataType.KEGG_COMPOUND, ValidityRule({"exact_mass": {"min": 0, "type": "numeric"}})
        )

        # NCBI Genome rules
        self.add_rule(
            DataType.NCBI_GENOME, CompletenessRule(["assembly_accession", "organism_name"], 0.95)
        )
        self.add_rule(
            DataType.NCBI_GENOME, AccuracyRule({"assembly_accession": r"^GC[AF]_\d{9}\.\d+$"})
        )
        self.add_rule(
            DataType.NCBI_GENOME, ValidityRule({"genome_size": {"min": 0, "type": "numeric"}})
        )

        # AGORA2 Model rules
        self.add_rule(
            DataType.AGORA2_MODEL, CompletenessRule(["model_id", "organism", "taxonomy"], 0.90)
        )
        self.add_rule(
            DataType.AGORA2_MODEL,
            ValidityRule(
                {
        ...
        self.add_rule(
            DataType.AGORA2_MODEL, OutlierDetectionRule(["reactions", "metabolites", "genes"])
        )
...
class QualityAnalyzer:
    ...
    def calculate_completeness(
        self, data: pd.DataFrame, required_fields: List[str] = None
    ) -> float:
    ...
    def calculate_accuracy(
        self, data: pd.DataFrame, validation_patterns: Dict[str, str] = None
    ) -> float:
        ...
        for field, field_constraints in constraints.items():
            if field in data.columns:
                ...
                if "min" in field_constraints:
                    violations = (
                        pd.to_numeric(field_data, errors="coerce") < field_constraints["min"]
                    ).sum()
                ...
                if "max" in field_constraints:
                    violations = (
                        pd.to_numeric(field_data, errors="coerce") > field_constraints["max"]
                    ).sum()
                    violation_count += violations
                    total_checks += len(field_data)
    ...
    def _parse_fcs_report(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "quality_score": 1.0
                - min(1.0, total_contaminated_length / 1000000),  # Normalize by 1Mb
            }
...
class QualityMonitor:
    ...
    def assess_quality(
        ...
        if isinstance(data, pd.DataFrame):
            ...
            required_fields = (
                validation_config.get("required_fields", []) if validation_config else []
            )
            validation_patterns = (
                validation_config.get("validation_patterns", {}) if validation_config else {}
            )
    ...
    def _generate_recommendations(
        ...
        if high_issues:
            recommendations.append(
                f"Prioritize resolution of {len(high_issues)} high-severity issues"
            )
        ...
        if auto_fixable:
            recommendations.append(
                f"Consider auto-fixing {len(auto_fixable)} automatically resolvable issues"
            )
    ...
    def _check_compliance(
        ...
        compliance["nasa_grade"] = (
            ...
            and metrics.accuracy >= 0.95
            and metrics.consistency >= 0.90
            and metrics.validity >= 0.95
        )
        ...
        compliance["research_grade"] = (
            ...
            and metrics.consistency >= 0.85
            and metrics.validity >= 0.90
        )
    ...
    def generate_quality_dashboard(
        self, output_path: str = "data/quality/dashboard.json"
    ) -> Dict[str, Any]:
