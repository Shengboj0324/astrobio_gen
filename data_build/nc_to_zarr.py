...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("nc_to_zarr.log"), logging.StreamHandler(sys.stdout)],
)
...
class MemoryOptimizer:
    ...
    def get_optimal_chunks(self, dataset: xr.Dataset) -> Dict[str, int]:
        ...
        dataset_size = sum(
            var.nbytes for var in dataset.data_vars.values() if hasattr(var, "nbytes")
        )
...
class ZarrConverter:
    ...
    def preprocess_dataset(self, ds: xr.Dataset) -> xr.Dataset:
        ...
        coord_mapping = {"latitude": "lat", "longitude": "lon", "level": "lev", "pressure": "lev"}
    ...
    def convert_file(self, nc_file: Path) -> bool:
        ...
        try:
            ...
            with xr.open_dataset(nc_file, chunks="auto") as ds:
                    ...
                    for var in ds.data_vars:
                        encoding[var] = {
                            "compressor": zarr.Blosc(
                                cname="lz4", clevel=self.config.compression_level
                            ),
                            "chunks": tuple(
                                optimal_chunks.get(dim, ds[var].sizes[dim]) for dim in ds[var].dims
                            ),
                        }
                ...
                logger.info(
                    f"✓ Converted {nc_file} in {processing_time:.2f}s ({file_size_gb:.2f} GB)"
                )
    ...
    def convert_all(self) -> None:
        ...
        try:
            ...
            with ProcessPoolExecutor(max_workers=self.config.parallel_workers) as executor:
                futures = {
                    executor.submit(self.convert_file, nc_file): nc_file for nc_file in nc_files
                }

                for future in tqdm(
                    as_completed(futures), total=len(futures), desc="Converting files"
                ):
...
def main():
    ...
    parser.add_argument(
        "--chunk", required=True, help="Chunk specification (e.g., 'lat:40,lon:40,lev:15,time:4')"
    )
