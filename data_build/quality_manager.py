...
class QualityMetrics:
    ...
    def overall_score(self) -> float:
        ...
        core_metrics = [self.completeness, self.consistency, self.accuracy, self.validity, self.uniqueness]
    ...
    def nasa_grade(self) -> str:
        ...
        elif score >= 0.90: return "A (Excellent)"
        elif score >= 0.80: return "B (Good)"
        elif score >= 0.70: return "C (Acceptable)"
        ...
        try:
            if self.rule_type == 'range':
                ...
                if isinstance(data, (int, float)):
                    ...
                    message = f"Value {data} outside range [{min_val}, {max_val}]" if not valid else "OK"
                else:
                    valid = False
                ...
                valid = func(data)
                message = self.parameters.get('message', 'Custom validation failed') if not valid else "OK"
    ...
    def _scientific_validation(self, data: Any) -> Tuple[bool, str]:
        ...
        if validation_type == 'stellar_temperature':
            ...
            if isinstance(data, (int, float)):
                ...
                message = f"Stellar Teff {data}K outside physical range [500-50000]K" if not valid else "OK"
            else:
                valid = False
            ...
            if isinstance(data, (int, float)):
                ...
                message = f"Planet radius {data} R_earth outside range [0.1-20]" if not valid else "OK"
            else:
                valid = False
...
class AdvancedDataQualityManager:
    ...
    def assess_data_quality(self, data: Union[pd.DataFrame, Dict, Path], data_type: str) -> QualityMetrics:
        """
    ...
    def filter_high_quality_data(
        ...
        logger.info(f"Filtered {data_type}: {original_size} → {filtered_size} records ({retention_rate:.1%} retained)")
    ...
    def process_exoplanet_data(self) -> Tuple[pd.DataFrame, QualityMetrics]:
        ...
        # Load exoplanet data
        planet_files = list(self.raw_data_path.glob("*exoplanet*")) + list(self.raw_data_path.glob("*planet*"))
            ...
            if rule.severity == 'error':
                ...
                for col in df.columns:
                    if col in rule.parameters.get('columns', [col]):
                        ...
                        violation_rate = violations / len(df[col].dropna()) if len(df[col].dropna()) > 0 else 0
    ...
    def _generate_quality_flags(
        ...
        # Outlier flags
        outlier_rate = metrics.flagged_records / metrics.total_records if metrics.total_records > 0 else 0
        if outlier_rate > 0.1:
    ...
    def _generate_recommendations(self, metrics: QualityMetrics, data_type: str) -> List[str]:
        ...
        if metrics.completeness < 0.9:
            recommendations.append("Increase data completeness by filling missing values or acquiring more complete datasets")
        
        if metrics.consistency < 0.8:
            recommendations.append("Improve data consistency by standardizing formats and removing duplicates")
        
        if metrics.accuracy < 0.9:
            recommendations.append("Enhance accuracy by implementing stricter validation rules and cross-checking with reference datasets")
        
        if metrics.flagged_records > metrics.total_records * 0.1:
            recommendations.append("Investigate flagged records for systematic issues and consider additional filtering")
        
        if metrics.overall_score < 0.8:
            recommendations.append("Overall quality below NASA standards - comprehensive data review recommended")
    ...
    def _assess_uncertainty(self, df: pd.DataFrame) -> Optional[float]:
        ...
        uncertainty_cols = [col for col in df.columns if 'err' in col.lower() or 'unc' in col.lower()]
