...
INT.mkdir(exist_ok=True, parents=True)

# -------- read KEGG tables (TAB-separated) ----------
pathways = pd.read_csv(RAW / "kegg_pathways.csv", sep="\t", names=["pathway", "desc"], dtype=str)

tags = pd.read_csv(INT / "pathway_env_tag.csv")  # comma-sep

hsa = pd.read_csv(
    RAW / "kegg_hsa_genes.csv", sep="\t", names=["gene_id", "description"], dtype=str
...
pd.DataFrame(env_rows.values(), columns=["pathway", "pH", "temp", "O2", "redox"]).to_csv(
    INT / "env_vectors.csv", index=False
)
