...
class UniProtDownloader:
    ...
    async def download_complete_database(self, database: str = "sprot", format: str = "dat") -> Optional[str]:
    ...
    async def download_taxonomic_division(self, database: str = "sprot", 
                                        division: str = "bacteria", 
                                        format: str = "dat") -> Optional[str]:
        ...
        try:
            # Construct download URL
            url = urljoin(self.base_url, f"current_release/knowledgebase/taxonomic_divisions/{filename}")
...
class UniProtParser:
    ...
    def _initialize_database(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_taxonomic_division ON protein_entries(taxonomic_division)')
    ...
    def parse_dat_file(self, file_path: str, taxonomic_division: str = "", max_entries: Optional[int] = None) -> List[UniProtEntry]:
                ...
                for line in f:
                    ...
                    if line.startswith('ID   '):
                        ...
                        if comment.startswith('-!- FUNCTION:'):
                            ...
                            current_entry.subcellular_location = comment.replace('-!- SUBCELLULAR LOCATION:', '').strip()
                    ...
                    elif line.startswith('     ') and current_entry and current_entry.sequence_length > 0:
                        # Sequence data
                        ...
                        if current_entry.accession:
                            # Set database type
                            current_entry.database = "Swiss-Prot" if taxonomic_division in ['sprot'] else "TrEMBL"
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            for entry in entries:
                try:
                    ...
                    ''', (
                        entry.accession, entry.entry_name, entry.protein_name,
                        entry.gene_name, entry.organism, entry.organism_id,
                        json.dumps(entry.taxonomic_lineage), entry.sequence,
                        entry.sequence_length, entry.molecular_weight,
                        entry.function, entry.subcellular_location, entry.pathway,
                        json.dumps(entry.keywords), json.dumps(entry.go_terms),
                        json.dumps(entry.ec_numbers), json.dumps(entry.kegg_ids),
                        json.dumps(entry.bigg_ids), json.dumps(entry.reactome_ids),
                        json.dumps(entry.pdb_ids), entry.database,
                        entry.taxonomic_division, entry.reviewed,
                        entry.annotation_score, entry.evidence_level,
                        entry.last_updated, json.dumps(entry.metadata)
                    ))
                except Exception as e:
...
class UniProtEMBLIntegration:
    ...
    async def download_taxonomic_data(self, divisions: Optional[List[str]] = None, 
                                    max_entries_per_division: Optional[int] = None) -> Dict[str, Any]:
            ...
            for division in divisions:
                ...
                if sprot_file:
                    ...
                    entries = self.parser.parse_dat_file(
                        sprot_file, taxonomic_division=division,
                ...
                trembl_limit = min(max_entries_per_division or 10000, 10000) if max_entries_per_division else 5000
                ...
                if trembl_file:
                    ...
                    entries = self.parser.parse_dat_file(
                        trembl_file, taxonomic_division=division,
                        max_entries=trembl_limit
                    )
    ...
    async def run_full_integration(self, divisions: Optional[List[str]] = None,
                                 max_entries_per_division: Optional[int] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"UniProt/EMBL-EBI integration completed in {end_time - start_time:.2f} seconds")
