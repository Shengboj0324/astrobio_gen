...
class StorageConfig:
    ...
    zarr_chunks: Dict[str, int] = field(
        default_factory=lambda: {"time": 10, "lat": 32, "lon": 32, "lev": 10}
    )
...
class MultiModalStorage:
    ...
    async def store_climate_datacube(
        self, run_id: int, datacube: xr.Dataset, variables: List[str] = None
    ) -> Path:
        """
        ...
        # Save to Zarr
        datacube.to_zarr(
            zarr_path, mode="w", encoding=encoding, consolidated=True  # Faster metadata access
        )
    ...
    async def load_climate_datacube(
        self,
        run_id: int,
        variables: List[str] = None,
        time_slice: slice = None,
        spatial_box: Tuple[slice, slice] = None,
    ) -> xr.Dataset:
        """
    ...
    async def store_biological_network(
        self, run_id: int, network_data: Dict[str, Any], network_type: str = "metabolic"
    ) -> Path:
        """
        ...
        if "graph" in network_data and hasattr(network_data["graph"], "nodes"):
            ...
            node_features = np.array([graph.nodes[n].get("features", []) for n in graph.nodes()])
            edge_features = np.array([[graph.edges[e].get("weight", 1.0)] for e in graph.edges()])
    ...
    async def load_biological_network(
        self, run_id: int, network_type: str = "metabolic", return_graph: bool = False
    ) -> Dict[str, Any]:
        """
    ...
    async def load_spectrum(
        self, run_id: int, instrument: str = "PSG", wavelength_range: Tuple[float, float] = None
    ) -> Dict[str, Any]:
        """
        ...
        # Get data path
        hdf5_path = self._get_data_path(
            run_id, DataDomain.SPECTROSCOPY, f"spectrum_{instrument.lower()}"
        )
        ...
        result = {"wavelengths": wavelengths, "flux": flux, "metadata": metadata}
    ...
    async def store_observation(
        ...
        fits_path = (
            run_dir
            / "observations"
            / f"{telescope.lower()}_{instrument.lower()}_{observation_type}.fits"
        )
    ...
    def _get_data_path(
        self, run_id: int, domain: DataDomain, data_type: str = None
    ) -> Optional[Path]:
        ...
        try:
            ...
            with self.planet_run_manager.get_session() as session:
                from .planet_run_primary_key_system import DataFile, PlanetRun

                query = session.query(DataFile).filter_by(run_id=run_id, domain=domain.value)
    ...
    def _evict_cache_items(self):
        ...
        sorted_items = sorted(self._access_stats.items(), key=lambda x: x[1]["last_access"])
    ...
    def get_storage_statistics(self) -> Dict[str, Any]:
        ...
        for domain in DataDomain:
            ...
            stats["domain_stats"][domain.value] = {"estimated_files": 0, "estimated_size_gb": 0.0}
        ...
        try:
            ...
            with self.planet_run_manager.get_session() as session:
                ...
                for data_file in old_files:
                    ...
                    if file_path.exists() and file_path.suffix not in [
                        ".zarr"
                    ]:  # Skip zarr (already compressed)
                        try:
                            ...
                            file_path.unlink()
                            data_file.file_path = str(
                                compressed_path.relative_to(self.config.storage_root)
                            )
    ...
    async def _migrate_to_cold_storage(self) -> Dict[str, Any]:
        ...
        try:
            ...
            with self.planet_run_manager.get_session() as session:
                ...
                old_access_keys = [
                    key
                    for key, stats in self._access_stats.items()
                    if (current_time.timestamp() - stats["last_access"])
                    > access_threshold.total_seconds()
                ]

                # Also check database for files with old access times
                old_files = (
                    session.query(DataFile)
                    .filter(DataFile.last_accessed < (current_time - access_threshold))
                    .all()
                )
                ...
                for data_file in old_files:
                    ...
                    if file_path.exists() and not str(file_path).startswith(str(cold_storage_dir)):
                        try:
                            ...
                            data_file.file_path = str(
                                cold_path.relative_to(self.config.storage_root)
                            )
                ...
                for data_file in all_files:
                    ...
                    if file_path.exists():
                        try:
                            ...
                            is_valid = await self._validate_file_integrity(
                                file_path, data_file.file_format
                            )
                            checked_files += 1
...
if __name__ == "__main__":
    ...
    async def test_storage():
        # Initialize storage
        config = StorageConfig(storage_root=Path("data/test_planet_runs"), max_memory_cache_gb=2.0)
