...
class GTDBDownloader:
    ...
    def __init__(self, base_url: str = None):
        ...
        self.mirror_url = (
            "https://data.ace.uq.edu.au/public/gtdb/data/releases/latest/"  # Traditional mirror
        )
    ...
    async def _get_managed_url(self, test_url: str) -> str:
        ...
        try:
            if self.url_system:
                managed_url = await self.url_system.get_url(test_url, priority=self.data_priority)
                if managed_url:
                    return managed_url
        except Exception as e:
...
class GTDBParser:
    ...
    def _initialize_database(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_gtdb_representative ON genomes(gtdb_representative)"
            )
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_taxonomy_rank ON taxonomy(rank)")
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_taxonomy_parent ON taxonomy(parent_taxon)"
            )
            ...
            for _, row in df.iterrows():
                try:
                    ...
                    genome = GTDBGenome(
                        ...
                        checkm_strain_heterogeneity=self._safe_float(
                            row.get("checkm_strain_heterogeneity", 0)
                        ),
                        # Gene content
                        ...
                        gtdb_species_representative=bool(
                            row.get("gtdb_species_representative", False)
                        ),
                ...
                try:
                    ...
                    """,
                        (
                            ...
                            genome.total_gap_length,
                            genome.checkm_completeness,
                            genome.checkm_contamination,
                            genome.checkm_strain_heterogeneity,
                            ...
                            genome.refseq_category,
                            genome.genbank_assembly_accession,
                            genome.refseq_assembly_accession,
                            genome.environment,
                            genome.isolation_source,
                            genome.gtdb_representative,
                            genome.gtdb_species_representative,
                            genome.gtdb_type_material,
                            genome.gtdb_type_designation,
                            genome.gtdb_release,
                            genome.last_updated,
                ...
                try:
                    ...
                    """,
                        (
                            ...
                            taxon.parent_taxon,
                            taxon.genome_count,
                            taxon.representative_genome,
                            taxon.branch_length,
                            taxon.node_support,
                            taxon.is_proposed_name,
                            taxon.is_gtdb_species_representative,
                            taxon.classification_notes,
                            json.dumps(taxon.child_taxa),
                            json.dumps(taxon.genomes),
    ...
    def generate_statistics(self) -> Dict[str, Any]:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                # Taxonomic diversity
                cursor.execute(
                    "SELECT COUNT(DISTINCT gtdb_phylum) FROM genomes WHERE gtdb_phylum != ''"
                )
                stats["phyla_count"] = cursor.fetchone()[0]

                cursor.execute(
                    "SELECT COUNT(DISTINCT gtdb_species) FROM genomes WHERE gtdb_species != ''"
                )
...
class GTDBIntegration:
    ...
    async def download_and_process_data(
        self,
        domains: List[str] = ["bacteria", "archaea"],
        max_genomes_per_domain: Optional[int] = None,
    ) -> Dict[str, Any]:
            ...
            for domain in domains:
                ...
                if domain_data:
                    ...
                    if metadata_key in domain_data:
                        genomes = self.parser.parse_metadata_file(domain_data[metadata_key], domain)
    ...
    async def run_full_integration(
        ...
        try:
            ...
            processing_results = await self.download_and_process_data(
                domains=domains, max_genomes_per_domain=max_genomes_per_domain
            )
...
async def main():
    ...
    results = await integration.run_full_integration(
        domains=["bacteria", "archaea"], max_genomes_per_domain=5000
    )
