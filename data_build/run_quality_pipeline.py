...
class PracticalDataCleaner:
    ...
    def clean_kegg_pathways(self) -> Tuple[pd.DataFrame, Dict]:
        ...
        edges_df = edges_df[
            (edges_df["substrate"].str.len() > 0) & (edges_df["product"].str.len() > 0)
        ]
        ...
        edges_df = edges_df[
            ~edges_df["substrate"].isin(generic_terms) & ~edges_df["product"].isin(generic_terms)
        ]
        ...
        logger.info(
            f"   [OK] Cleaned KEGG data: {results['original_count']} → {results['cleaned_count']} edges"
        )
    ...
    def clean_genomic_data(self) -> Tuple[pd.DataFrame, Dict]:
        ...
        if before_filter > after_filter:
            results["quality_issues"].append(
                f"Removed {before_filter - after_filter} incomplete records"
            )
        ...
        if before_dedup > after_dedup:
            results["quality_issues"].append(
                f"Removed {before_dedup - after_dedup} duplicate samples"
            )
        ...
        logger.info(
            f"   [OK] Processed genomic metadata: {results['sequences_processed']} sequences"
        )
    ...
    def generate_quality_summary(self, all_results: Dict) -> Dict:
        ...
        if summary["overall_quality_score"] < 0.90:
            summary["recommendations"].append(
                "Quality score below NASA standards (0.90). Review data collection and filtering processes."
            )

        if summary["overall_quality_score"] < 0.80:
            summary["recommendations"].append(
                "Significant quality issues detected. Consider additional data sources or stricter validation."
            )
