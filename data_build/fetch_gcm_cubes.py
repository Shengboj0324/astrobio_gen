    ...
    async def monitor_and_convert(self, check_interval: int = 300):
        ...
        while True:
            try:
                ...
                logger.info(f"Progress: {progress['runs_completed']}/{progress['runs_submitted']} completed, "
                ...
                if (progress['runs_submitted'] > 0 and 
                    progress['runs_converted'] >= progress['runs_submitted']):
...
def main():
    ...
    parser.add_argument('--action', choices=['submit', 'monitor', 'convert', 'status'], 
