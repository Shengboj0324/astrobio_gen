...
class RobustDataQualityManager:
    ...
    def clean_kegg_edges_robust(self) -> Tuple[pd.DataFrame, Dict]:
        ...
        edges_df['substrate'] = edges_df['substrate'].astype(str).str.replace('cpd:', '', regex=False)
        ...
        if removed_invalid > 0:
            results['quality_issues'].append(f"Removed {removed_invalid} invalid substrate/product pairs")
    ...
    def clean_environmental_vectors_robust(self) -> Tuple[pd.DataFrame, Dict]:
        ...
        env_combinations = env_df.groupby(['pH', 'temp', 'O2', 'redox']).size().reset_index(name='count')
        ...
        if len(unique_conditions) >= len(env_df) * 0.1:  # At least 10% unique
            ...
            if removed_duplicates > 0:
                results['quality_issues'].append(f"Removed {removed_duplicates} duplicate conditions")
        ...
        if len(env_df) >= 10:
            ...
            if len(outlier_indices) <= len(env_df) * 0.1:  # Max 10% outliers
                ...
                if len(outlier_indices) > 0:
                    results['quality_issues'].append(f"Removed {len(outlier_indices)} statistical outliers")
        ...
        env_analysis = {
            ...
            'unique_combinations': len(env_df.drop_duplicates(subset=['pH', 'temp', 'O2', 'redox'])),
            'parameter_stats': {
                'pH': {'min': env_df['pH'].min(), 'max': env_df['pH'].max(), 'mean': env_df['pH'].mean()},
                'temp': {'min': env_df['temp'].min(), 'max': env_df['temp'].max(), 'mean': env_df['temp'].mean()},
                'O2': {'min': env_df['O2'].min(), 'max': env_df['O2'].max(), 'mean': env_df['O2'].mean()},
                'redox': {'min': env_df['redox'].min(), 'max': env_df['redox'].max(), 'mean': env_df['redox'].mean()}
            }
        }
    ...
    def clean_genomic_data_robust(self) -> Tuple[pd.DataFrame, Dict]:
        ...
        if before_filter > after_filter:
            results['quality_issues'].append(f"Removed {before_filter - after_filter} entries with missing sample IDs")
        ...
        if before_dedup > after_dedup:
            results['quality_issues'].append(f"Removed {before_dedup - after_dedup} duplicate sample IDs")
        ...
        genome_df['completeness_score'] = (
            genome_df['has_file_path'].astype(int) +
            genome_df['has_file_size'].astype(int)
        ) / 2.0
    ...
    def generate_comprehensive_report(self, all_results: Dict) -> Dict:
        ...
        for dataset_name, results in all_results.items():
            if 'error' in results:
                ...
                summary['recommendations'].append(f"Fix errors in {dataset_name}: {results['error']}")
            else:
                ...
                if 'cleaned_count' in results and 'original_count' in results:
                    ...
                    retention_rate = 1.0  # Genomic data processing doesn't have original/cleaned distinction
                else:
                    retention_rate = 0.0
                ...
                if quality_score < 0.8:
                    summary['recommendations'].append(f"Improve {dataset_name} quality (current: {quality_score:.1%})")
        ...
        # Assessments
        summary['nasa_readiness'] = summary['overall_quality_score'] >= 0.85  # Slightly more lenient
        summary['data_ready_for_ml'] = summary['overall_quality_score'] >= 0.70  # ML-ready threshold
        ...
        if summary['overall_quality_score'] < 0.85:
            summary['recommendations'].append("Consider relaxing quality thresholds if data is scientifically valid")
