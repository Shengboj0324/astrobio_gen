...
class GEMsDownloader:
    ...
    async def download_sample_genomes(
        self, sample_ids: List[str], max_genomes: Optional[int] = None
    ) -> Dict[str, str]:
                ...
                try:
                    async with session.get(url) as response:
                        if response.status == 200:
                            ...
                            logger.warning(
                                f"Could not download genome {sample_id}: HTTP {response.status}"
                            )
                except Exception as e:
...
class GEMsParser:
    ...
    def _initialize_database(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_genome_quality ON genomes(quality_level)"
            )
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_genome_phylum ON genomes(phylum)")
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_genome_environment ON genomes(environment_type)"
            )
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_genome_novel ON genomes(is_novel_species)"
            )
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_metagenome_ecosystem ON metagenomes(ecosystem)"
            )
                        ...
                        if member.name.endswith(".tsv") or member.name.endswith(".csv"):
                            ...
                            if f:
                                ...
                                if "genome" in member.name.lower():
                                    ...
                                    "sample" in member.name.lower()
                                    or "metagenome" in member.name.lower()
                                ):
                ...
                try:
                    ...
                    """,
                        (
                            ...
                            genome.quality_score,
                            genome.quality_level,
                            genome.genome_size,
                            genome.contig_count,
                            genome.n50,
                            genome.gc_content,
                            ...
                            genome.genus,
                            genome.species,
                            genome.gtdb_taxonomy,
                            genome.ncbi_taxonomy,
                            genome.is_novel_species,
                            genome.is_candidate_species,
                            genome.gene_count,
                            genome.protein_count,
                            genome.rrna_5s,
                            genome.rrna_16s,
                            genome.rrna_23s,
                            genome.trna_count,
                            genome.environment_type,
                            genome.habitat,
                            genome.sampling_site,
                            genome.geographic_location,
                            genome.metagenome_source,
                            genome.sequencing_method,
                            genome.assembly_method,
                            genome.binning_method,
                            genome.genome_fasta_file,
                            genome.annotation_file,
                            genome.protein_file,
                            genome.metadata_file,
                            genome.biosample_id,
                            genome.study_id,
                            genome.jgi_project_id,
                            genome.submission_date,
                            genome.last_updated,
                ...
                try:
                    ...
                    """,
                        (
                            ...
                            metagenome.sample_name,
                            metagenome.study_name,
                            metagenome.ecosystem,
                            metagenome.ecosystem_category,
                            metagenome.ecosystem_type,
                            metagenome.specific_ecosystem,
                            metagenome.latitude,
                            metagenome.longitude,
                            metagenome.elevation,
                            metagenome.geographic_location,
                            metagenome.sampling_date,
                            metagenome.depth,
                            metagenome.temperature,
                            metagenome.ph,
                            metagenome.salinity,
                            metagenome.sequencing_platform,
                            metagenome.sequencing_method,
                            metagenome.read_count,
                            metagenome.total_bases,
                            metagenome.average_read_length,
                            metagenome.assembly_size,
                            metagenome.contig_count,
                            metagenome.assembly_n50,
                            metagenome.total_bins,
                            metagenome.high_quality_bins,
                            metagenome.medium_quality_bins,
                            metagenome.low_quality_bins,
                            metagenome.raw_data_file,
                            metagenome.assembly_file,
                            metagenome.annotation_file,
                            metagenome.bins_directory,
                            json.dumps(metagenome.metadata),
                        ),
                    )
                except Exception as e:
    ...
    def generate_statistics(self) -> Dict[str, Any]:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                cursor.execute(
                    "SELECT COUNT(DISTINCT ecosystem) FROM metagenomes WHERE ecosystem != ''"
                )
...
class JGIGEMsIntegration:
    ...
    async def download_sample_genomes(
        self, sample_ids: List[str], max_downloads: Optional[int] = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            downloaded_files = await self.downloader.download_sample_genomes(
                sample_ids, max_downloads
            )
    ...
    async def run_full_integration(
        self, max_genomes: Optional[int] = None, download_genome_files: bool = False
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if (
                ...
                and catalog_results.get("processing", {}).get("genomes", 0) > 0
            ):
                # Get sample IDs from stored genomes
                with sqlite3.connect(self.parser.db_path) as conn:
                    cursor = conn.cursor()
                    cursor.execute(
                        "SELECT DISTINCT sample_id FROM genomes WHERE quality_level = 'high' LIMIT 100"
                    )
                    sample_ids = [row[0] for row in cursor.fetchall() if row[0]]

                if sample_ids:
                    genome_results = await self.download_sample_genomes(
                        sample_ids, max_downloads=50
                    )
            ...
            logger.info(
                f"Total genomes processed: {results.get('statistics', {}).get('total_genomes', 0)}"
            )
...
async def main():
    ...
    results = await integration.run_full_integration(max_genomes=1000, download_genome_files=False)
