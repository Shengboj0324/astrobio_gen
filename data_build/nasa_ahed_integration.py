...
class AHEDDownloader:
        ...
        if self.url_system:
            try:
                ...
                managed_url = await self.url_system.get_url(
                    "https://ahed.nasa.gov/", self.data_priority
                )
                if managed_url:
    ...
    async def search_datasets(
        ...
        try:
            ...
            search_params = {"format": "json", "limit": max_results}
    ...
    async def _scrape_dataset_listings(
        self,
        theme: Optional[str] = None,
        keywords: Optional[List[str]] = None,
        max_results: int = 100,
        ...
        try:
            ...
            async with session.get(datasets_url) as response:
                if response.status == 200:
                    ...
                    enhanced_datasets = await self._enhance_dataset_metadata(
                        datasets[:20]
    ...
    async def _enhance_dataset_metadata(
            ...
            try:
                ...
                async with session.get(dataset["url"]) as response:
                    if response.status == 200:
                        ...
                        if authors_elem:
                            enhanced_dataset["authors"] = [
                                a.strip() for a in authors_elem.get_text().split(",")
                            ]
                        ...
                        for link in download_links:
                            ...
                            if any(
                                ext in href.lower()
                                for ext in [".csv", ".json", ".xml", ".zip", ".gz", ".nc", ".hdf"]
                            ):
                                files.append(
                                    {
                        ...
                        geo_elem = soup.find(
                            "div", class_=["location", "geographic", "coordinates"]
                        )
                        if geo_elem:
                            geo_text = geo_elem.get_text()
                            # Try to extract coordinates
                            lat_match = re.search(
                                r"lat(?:itude)?[:\s]*([+-]?\d+\.?\d*)", geo_text, re.I
                            )
                            lon_match = re.search(
                                r"lon(?:gitude)?[:\s]*([+-]?\d+\.?\d*)", geo_text, re.I
                            )
                            if lat_match:
    ...
    async def download_dataset_files(
        self, dataset: Dict[str, Any], max_files: Optional[int] = None
    ) -> Dict[str, str]:
...
class AHEDParser:
    ...
    def _initialize_database(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_dataset_environment ON datasets(environment_type)"
            )
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_dataset_location ON datasets(latitude, longitude)"
            )
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_site_ecosystem ON field_sites(ecosystem_type)"
            )
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_site_location ON field_sites(latitude, longitude)"
            )
                ...
                try:
                    ...
                    """,
                        (
                            ...
                            dataset.description,
                            dataset.theme,
                            json.dumps(dataset.keywords),
                            json.dumps(dataset.authors),
                            dataset.institution,
                            dataset.contact_email,
                            dataset.collection_start_date,
                            dataset.collection_end_date,
                            ...
                            dataset.ecosystem,
                            dataset.temperature_range,
                            dataset.ph_range,
                            dataset.salinity,
                            dataset.radiation_environment,
                            json.dumps(dataset.data_types),
                            json.dumps(dataset.file_formats),
                            dataset.data_size,
                            json.dumps(dataset.instruments),
                            json.dumps(dataset.missions),
                            dataset.quality_level,
                            dataset.processing_level,
                            dataset.calibration_status,
                            dataset.validation_status,
                            dataset.access_level,
                            dataset.download_url,
                            dataset.doi,
                            dataset.funding_agency,
                            json.dumps(dataset.grants),
                ...
                try:
                    ...
                    """,
                        (
                            ...
                            site.biosignature_potential,
                            site.geological_setting,
                            site.hydrology,
                            json.dumps(site.chemistry),
                            json.dumps(site.active_studies),
                            json.dumps(site.available_datasets),
                            site.accessibility,
                            site.permits_required,
                            site.contact_info,
...
class NASAAHEDIntegration:
    ...
    async def search_and_download_data(
        self,
        ...
        try:
            ...
            for theme in themes:
                ...
                datasets = await self.downloader.search_datasets(
                    theme=theme, max_results=max_datasets_per_theme
                )
    ...
    async def run_full_integration(
        ...
        try:
            ...
            logger.info(
                f"Total datasets processed: {results.get('statistics', {}).get('total_datasets', 0)}"
            )
