...
class ComprehensiveDataSystemDemo:
    ...
    async def demonstrate_enhanced_kegg_crawling(self) -> Dict[str, Any]:
        ...
        if sample_pathway:
            ...
            if pathway_details:
                results['sample_pathway'] = {
                    ...
                    'cross_references': {k: len(v) for k, v in pathway_details.cross_references.items()},
        ...
        quality_report = self.quality_monitor.assess_quality(
            pd.DataFrame(pathways[:50]), 
            "kegg_pathways", 
            DataType.KEGG_PATHWAY
        )
    ...
    async def demonstrate_enhanced_ncbi_crawling(self) -> Dict[str, Any]:
        ...
        for domain in sample_domains:
            try:
                assemblies = await self.ncbi_integration.ncbi_downloader.fetch_assembly_summary(domain)
        ...
        results['supported_file_types'] = {
            ...
            'quality_control_files': [k for k in available_files.keys() if 'fcs' in k or 'ani' in k],
            'sequence_files': [k for k in available_files.keys() if 'genomic' in k or 'protein' in k],
            'annotation_files': [k for k in available_files.keys() if 'gff' in k or 'gtf' in k],
            'expression_files': [k for k in available_files.keys() if 'expression' in k or 'rnaseq' in k],
    ...
    async def demonstrate_comprehensive_quality_system(self) -> Dict[str, Any]:
        ...
        ncbi_quality_analysis = self.quality_monitor.analyzer.analyze_ncbi_quality_files(quality_files)
    ...
    async def run_full_demonstration(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.demo_results['quality_system'] = await self.demonstrate_comprehensive_quality_system()
