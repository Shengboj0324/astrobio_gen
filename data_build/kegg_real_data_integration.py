        ...
        try:
            if self.url_system:
                ...
                managed_url = await self.url_system.get_url(
                    self.base_url, priority=self.data_priority
                )
                if managed_url:
                    return managed_url
        except Exception as e:
    ...
    async def fetch_organism_list(self) -> List[Dict[str, str]]:
        ...
        for line in response.strip().split("\n"):
            ...
            if len(parts) >= 3:
                ...
                organisms.append(
                    {"org_code": org_code, "name": name, "scientific_name": scientific_name}
                )
        ...
        for line in response.strip().split("\n"):
            if line.startswith("dr:"):
                ...
                if len(parts) >= 2:
                    ...
                    name = parts[1]
                    drugs.append({"drug_id": drug_id, "name": name})
    ...
    def _categorize_pathway(self, pathway_id: str, name: str) -> Tuple[str, str]:
        ...
        # Human Diseases (map05000-map05999)
            ...
            if "cancer" in name.lower() or "tumor" in name.lower():
                ...
                "neurodegenerative" in name.lower()
                or "alzheimer" in name.lower()
                or "parkinson" in name.lower()
            ):
        ...
        with sqlite3.connect(self.processor.db_path) as conn:
            ...
            for drug in drugs:
                cursor.execute(
                    ...
                    VALUES (?, ?, ?)
                """,
                    (drug["drug_id"], drug["name"], datetime.now(timezone.utc)),
                )
