...
class MultiModalStorage:
    ...
    def _initialize_storage(self):
        ...
        directories = [self.config.storage_root, self.config.cache_root, self.config.temp_root]
    ...
    def _get_file_info(
        self, run_id: int, domain: DataDomain, data_type: str
    ) -> Optional[DataFileInfo]:
    ...
    async def load_climate_datacube(
        self, run_id: int, variables: List[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        result = {"data": data_dict, "dimensions": dimensions, "metadata": metadata}
    ...
    async def load_biological_network(
        self, run_id: int, network_type: str = "metabolic"
    ) -> Dict[str, Any]:
        """
        ...
        with self._cache_lock:
            ...
            if len(self._memory_cache) >= max_cache_items:
                ...
                if self._access_stats:
                    sorted_items = sorted(
                        self._access_stats.items(), key=lambda x: x[1]["last_access"]
                    )
...
async def create_example_data(storage: MultiModalStorage, run_id: int):
    ...
    climate_data = {"temperature": temp, "humidity": humidity}

    dimensions = {"time": time, "lat": lat, "lon": lon, "lev": lev}
    ...
    n_nodes = 50
    network_data = {
        "adjacency_matrix": np.random.choice([0, 1], size=(n_nodes, n_nodes), p=[0.9, 0.1]).astype(
            np.float32
        ),
    ...
    flux = (np.exp(-(((wavelengths - 10.0) / 5.0) ** 2)) + 0.1 * np.random.random(n_points)).astype(
        np.float32
    )
    ...
    await storage.store_spectrum(
        run_id, wavelengths, flux, resolution=100000, instrument="PSG", metadata=spectrum_metadata
    )
...
if __name__ == "__main__":
    ...
    async def test_storage():
        ...
        # Initialize storage
        config = StorageConfig(storage_root=Path("data/test_planet_runs"), max_memory_cache_gb=1.0)
        ...
        for run_id in stored_runs:
            ...
            print(
                f"  Run {run_id}: {run_info['file_count']} files, "
