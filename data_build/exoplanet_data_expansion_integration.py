...
class ExoplanetDataExpansionIntegrator:
    ...
    async def load_expansion_configuration(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"✅ Loaded expansion configuration: {config['metadata']['total_new_sources']} sources"
            )
    ...
    async def detect_conflicts(self, expansion_config: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        if conflicts:
            ...
            for conflict in conflicts:
                logger.warning(
                    f"   Conflict: {conflict['new_source']} vs {conflict['existing_source']} ({', '.join(conflict['conflict_types'])})"
                )
        else:
        ...
        async def validate_single_source(source_key, source_data):
            async with semaphore:
                ...
                if result["accessible"]:
                    logger.info(
                        f"✅ {result['source_name']}: Accessible ({result['response_time_ms']:.0f}ms)"
                    )
                else:
                    logger.warning(
                        f"❌ {result['source_name']}: Not accessible - {result.get('error', 'Unknown error')}"
                    )
            ...
            if isinstance(sources, dict) and category not in ["metadata", "integration", "summary"]:
                ...
                for source_key, source_data in sources.items():
                    if isinstance(source_data, dict) and "name" in source_data:
                        ...
                        # Size accumulation
                        if (
                            "metadata" in source_data
                            and "estimated_size_gb" in source_data["metadata"]
                        ):
                            stats["estimated_total_size_gb"] += source_data["metadata"][
                                "estimated_size_gb"
                            ]
                        ...
                        stats["sources_by_priority"][priority] = (
                            stats["sources_by_priority"].get(priority, 0) + 1
                        )
                        ...
                        if "metadata" in source_data and "quality_score" in source_data["metadata"]:
                            ...
                            stats["quality_score_distribution"][score_range] = (
                                stats["quality_score_distribution"].get(score_range, 0) + 1
                            )
    ...
    async def integrate_sources(
        self, expansion_config: Dict[str, Any], conflicts: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
                        ...
                        integration_results["successful_integrations"].append(
                            {
                                ...
                                "estimated_size_gb": source_data.get("metadata", {}).get(
                                    "estimated_size_gb", 0
                                ),
                            }
                        )
        ...
        logger.info(
            f"🎯 Integration complete: {self.stats['successfully_integrated']} sources integrated"
        )
        ...
        report = {
            "integration_summary": {
                ...
                "validation_success_rate": (
                    sum(1 for r in validation_results.values() if r["accessible"])
            ...
            "quality_assurance": {
                ...
                "accessibility_validation": {
                    ...
                    "accessible_sources": sum(
                        1 for r in validation_results.values() if r["accessible"]
                    ),
                    "average_response_time_ms": (
                        ...
                        / len([r for r in validation_results.values() if r.get("response_time_ms")])
                        if validation_results
                        else 0
                    ),
                },
    ...
    async def run_comprehensive_integration(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"✅ {final_report['integration_summary']['successful_integrations']} sources integrated"
            )
            logger.info(
                f"📊 {expansion_stats['estimated_total_size_gb']:.1f} GB additional data coverage"
            )
