...
try:
    ...
    print(
        "Note: This system now uses REAL scientific datasets with ZERO tolerance for dummy/synthetic data."
    )
    sys.exit(1)
...
class ComprehensiveDataSystem:
    ...
    async def initialize_components(self):
        ...
        try:
            self.logger.info(
                "[START] Initializing comprehensive data system components with enterprise URL management"
            )
            ...
            self.logger.info(
                f"[DATA] URL system validation: {health_status.get('summary', 'Unknown')}"
            )
            ...
            self.logger.info(
                "All components initialized successfully (including new high-quality data integrations)"
            )
    ...
    async def run_full_pipeline(self, limits: Dict[str, int] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            config = PipelineConfig(
                ...
                # Apply limits
                max_kegg_pathways=(
                    limits.get("kegg_pathways")
                    if limits
                    else self.config.get("max_kegg_pathways", 100)
                ),
                max_ncbi_genomes=(
                    limits.get("ncbi_genomes")
                    if limits
                    else self.config.get("max_ncbi_genomes", 50)
                ),
                max_agora2_models=(
                    limits.get("agora2_models")
                    if limits
                    else self.config.get("max_agora2_models", 50)
                ),
                # Quality settings
            ...
            summary = {
                ...
                "nasa_ahed_datasets": high_quality_results.get("nasa_ahed", {}).get(
                    "total_datasets", 0
                ),
    ...
    async def run_high_quality_integrations(self, limits: Dict[str, int] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            max_entries_per_division = (
                limits.get("uniprot_entries_per_division", 1000) if limits else None
            )
            ...
            try:
                ...
                self.logger.info(
                    f"[OK] UniProt integration completed: {uniprot_results.get('total_entries', 0)} entries"
                )
            except Exception as e:
            ...
            try:
                ...
                self.logger.info(
                    f"[OK] JGI GEMs integration completed: {jgi_results.get('statistics', {}).get('total_genomes', 0)} genomes"
                )
            except Exception as e:
            ...
            try:
                gtdb_results = await self.gtdb_integration.run_full_integration(
                    domains=["bacteria", "archaea"], max_genomes_per_domain=max_genomes_per_domain
                )
                results["gtdb"] = gtdb_results
                self.logger.info(
                    f"[OK] GTDB integration completed: {gtdb_results.get('statistics', {}).get('total_genomes', 0)} genomes"
                )
            except Exception as e:
            ...
            try:
                ...
                self.logger.info(
                    f"[OK] NASA AHED integration completed: {ahed_results.get('statistics', {}).get('total_datasets', 0)} datasets"
                )
            except Exception as e:
            ...
            self.logger.info(
                f"[SUCCESS] HIGH-QUALITY INTEGRATIONS COMPLETED: {successful_integrations}/{total_integrations} successful"
            )
    ...
    async def run_high_quality_only_mode(self) -> Dict[str, Any]:
        ...
        try:
            ...
            summary = {
                ...
                "duration_seconds": (
                    duration.total_seconds() if hasattr(duration, "total_seconds") else 0
                ),
                "dummy_data_removed": True,
                "real_data_only": True,
                "integration_results": results,
                    ...
                    for csv_file in csv_files:
                        try:
                            ...
                            report = self.quality_monitor.assess_quality(
                                data=df, data_source=str(csv_file.stem), data_type=data_type
                            )
                            ...
                            quality_results[str(csv_file)] = {"error": str(e), "overall_score": 0.0}

            # Generate summary
            if quality_results:
                scores = [
                    r["overall_score"] for r in quality_results.values() if "overall_score" in r
                ]
                nasa_ready_count = sum(
                    1 for r in quality_results.values() if r.get("nasa_ready", False)
                )
                ...
                summary = {
                    ...
                    "total_size_mb": sum(
                        r.get("file_size_mb", 0) for r in quality_results.values()
                    ),
            ...
            for item in root_path.rglob("*"):
                if item.is_file():
                    ...
                    structure["largest_files"].append(
                        {"path": str(item), "size_mb": size / (1024 * 1024)}
                    )
    ...
    def _analyze_database(self, db_path: Path) -> Dict[str, Any]:
        ...
        analysis = {"exists": False, "size_mb": 0, "tables": [], "total_records": 0}
    ...
    def _cleanup_temp_files(self) -> Dict[str, Any]:
        ...
        cleanup_results = {"files_removed": 0, "space_freed_mb": 0, "directories_cleaned": []}
    ...
    def _optimize_databases(self) -> Dict[str, Any]:
        ...
        optimization_results = {"optimized": [], "failed": [], "space_saved_mb": 0}
        ...
        for db_path in db_files:
            try:
                ...
                optimization_results["optimized"].append(
                    {"path": str(db_path), "space_saved_mb": space_saved}
                )
                optimization_results["space_saved_mb"] += space_saved

            except Exception as e:
                optimization_results["failed"].append({"path": str(db_path), "error": str(e)})
    ...
    def _cleanup_caches(self) -> Dict[str, Any]:
        ...
        cache_results = {"cleaned": [], "space_freed_mb": 0}
            ...
            if cache_dir.exists():
                try:
                    ...
                    cache_results["cleaned"].append(
                        {"path": str(cache_dir), "space_freed_mb": removed_size / (1024 * 1024)}
                    )
...
Examples:
  ...
  python run_comprehensive_data_system.py --mode quality --output results.json
    ...
    parser.add_argument(
        "--max-concurrent", type=int, default=4, help="Maximum concurrent tasks (default: 4)"
    )

    parser.add_argument(
        "--max-memory", type=int, default=16, help="Maximum memory usage in GB (default: 16)"
    )

    parser.add_argument("--kegg-limit", type=int, help="Limit KEGG pathways to download")

    parser.add_argument("--ncbi-limit", type=int, help="Limit NCBI genomes to download")

    parser.add_argument("--agora2-limit", type=int, help="Limit AGORA2 models to download")

    parser.add_argument(
        "--force", action="store_true", help="Force execution even if system requirements not met"
    )
