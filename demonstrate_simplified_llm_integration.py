...
class SimplifiedLLMSystem:
    ...
    def generate_rationale(self, surrogate_outputs: Dict[str, Any]) -> str:
        ...
        if habitability > 0.8:
            rationale_parts.append(
                f"This planet shows excellent habitability potential with a score of {habitability:.2f}."
            )
        elif habitability > 0.6:
            rationale_parts.append(
                f"The planet demonstrates promising habitability indicators (score: {habitability:.2f})."
            )
        elif habitability > 0.4:
            rationale_parts.append(
                f"Moderate habitability potential is observed with a score of {habitability:.2f}."
            )
        else:
            rationale_parts.append(
                f"The planet presents challenging habitability conditions (score: {habitability:.2f})."
            )

        # Temperature and pressure analysis
        if 0 <= temp_c <= 100:
            rationale_parts.append(
                f"Surface temperatures of {temp_c:.1f}°C support liquid water stability."
            )
        elif temp_c > 100:
            rationale_parts.append(
                f"Elevated surface temperatures ({temp_c:.1f}°C) may challenge habitability."
            )
        else:
            rationale_parts.append(
                f"Cold surface conditions ({temp_c:.1f}°C) limit liquid water availability."
            )

        if pressure > 0.1:
            rationale_parts.append(
                f"The substantial atmosphere ({pressure:.2f} bar) can retain heat and support weather systems."
            )
        else:
            rationale_parts.append(
                f"The thin atmosphere ({pressure:.3f} bar) provides limited surface pressure."
            )

        # Biosignature analysis
        if o2_snr > 5:
            rationale_parts.append(
                f"Strong oxygen detection (SNR: {o2_snr:.1f}) suggests potential photosynthetic activity."
            )
        elif o2_snr > 2:
            rationale_parts.append(
                f"Moderate oxygen signal (SNR: {o2_snr:.1f}) warrants further investigation."
            )

        if ch4_snr > 5:
            rationale_parts.append(
                f"Significant methane detection (SNR: {ch4_snr:.1f}) indicates atmospheric disequilibrium."
            )
        ...
        if uncertainty < 0.1:
            ...
            confidence_text = (
                f"Moderate confidence in predictions with {uncertainty:.1f} uncertainty."
            )
        else:
            confidence_text = (
                f"Results are preliminary with significant uncertainty (±{uncertainty:.1f})."
            )
            ...
            if context and "habitability_score" in context:
                ...
                interpretation = (
                    ...
                    if score > 0.8
                        ...
                        "promising" if score > 0.6 else "moderate" if score > 0.4 else "challenging"
                    )
                )
    ...
    def generate_voice_over(
        self, surrogate_outputs: Dict[str, Any], duration_target: int = 60
    ) -> str:
        ...
        if 0 <= temp_c <= 100:
            temp_analysis += (
                " fall within the liquid water range, a crucial requirement for life as we know it."
            )
        elif temp_c > 100:
            temp_analysis += (
                " indicate a hot world where liquid water would be challenging to maintain."
            )
        else:
            temp_analysis += (
                " suggest a cold environment requiring greenhouse warming for habitability."
            )
        ...
        if uncertainty < 0.1:
            confidence = (
                "Our models show high confidence in these predictions, with minimal uncertainty."
            )
        elif uncertainty < 0.2:
        ...
        script = (
            f"{opening} {main_finding} {temp_analysis} {atm_analysis} {confidence} {conclusion}"
        )
...
class AstrobiologyDemo:
    ...
    async def _test_function_1_rationale(self):
        ...
        for case in test_cases:
            ...
            logger.info(
                f"📊 Input: Habitability {case['data']['habitability_score']:.2f}, Temp {case['data']['surface_temperature']-273.15:.1f}°C"
            )
    ...
    async def _test_function_3_voice_over(self):
        ...
        for scenario in scenarios:
            ...
            result = {
                ...
                "generation_time_ms": generation_time,
                "duration_accuracy": abs(estimated_duration - scenario["duration"])
                / scenario["duration"],
            }
        ...
        self.results["function_3_voice_over"] = {
            ...
            "avg_generation_time_ms": np.mean(
                [r["generation_time_ms"] for r in voice_over_results]
            ),
        }
    ...
    async def _test_integration(self):
        ...
        surrogate_outputs = {
            ...
            "surface_temperature": 255 * (planet_params["insolation"] ** 0.25)
        ...
        logger.info(
            f"🌍 Planet: {planet_params['radius_earth']}R⊕, {planet_params['insolation']}S⊕"
        )
    ...
    async def _save_and_display_results(self):
        ...
        logger.info(f"\n🤝 Integration Test:")
        logger.info(
            f"   ✅ Surrogate model coordination: {'Success' if integration.get('integration_successful') else 'Failed'}"
        )
        logger.info(
            f"   ⚡ Complete analysis: {integration.get('total_generation_time_ms', 0):.1f}ms"
        )
