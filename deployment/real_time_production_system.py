...
if MONITORING_AVAILABLE:
    ...
    MODEL_INFERENCE_TIME = Histogram(
        "astrobio_model_inference_seconds", "Model inference time", ["model_type"]
    )
    DATA_PROCESSING_RATE = Gauge(
        "astrobio_data_processing_rate", "Data processing rate (samples/sec)"
    )
...
class StreamProcessor:
    ...
    async def _ingest_message(self, data: Dict[str, Any], source: str):
        ...
        self.current_batch.append(
            {"data": data, "timestamp": datetime.now(timezone.utc), "source": source}
        )
        ...
        batch_ready = (
            len(self.current_batch) >= self.config.batch_size
            or (time.time() - self.batch_start_time) * 1000 >= self.config.max_batch_wait_ms
        )
        ...
        while True:
            try:
                ...
                if processing_time > self.config.max_latency_ms:
                    logger.warning(
                        f"⚠️ Batch processing exceeded latency target: {processing_time:.1f}ms"
                    )
    ...
    async def _run_spectral_analysis(self, features: torch.Tensor) -> Dict[str, Any]:
        ...
        try:
            ...
            model = await self.model_cache.load_model(
                "spectral_analyzer", {"input_dim": features.shape[-1], "output_dim": 10}
            )
