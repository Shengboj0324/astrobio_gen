...
# Local imports 
from models.domain_encoders_simple import MultiModalEncoder, EncoderConfig
from data_build.unified_dataloader_fixed import (
    create_multimodal_dataloaders, DataLoaderConfig, MultiModalBatch
)
...
class TrainingConfig:
    ...
    initial_loss_weights: Dict[str, float] = field(default_factory=lambda: {
        'climate': 1.0,
        'spectrum': 0.3,
        'physics': 0.2,
        'consistency': 0.1
    ...
    # Physics constraints
    physics_loss_schedule: Dict[str, Any] = field(default_factory=lambda: {
        'start_epoch': 0,
        'ramp_epochs': 50,
        'max_weight': 1.0
    })
...
class MultiTaskLoss(nn.Module):
    ...
    def forward(self, 
        ...
        if 'climate_cubes' in targets and 'climate_prediction' in predictions:
            climate_loss = self._compute_climate_loss(
                predictions['climate_prediction'], 
                targets['climate_cubes']
            )
        ...
        if 'spectra' in targets and 'spectrum_prediction' in predictions:
            spectrum_loss = self._compute_spectrum_loss(
                predictions['spectrum_prediction'],
                targets['spectra']
            )
        ...
        if 'physics_constraints' in encoder_outputs:
            physics_loss = self._compute_physics_loss(
                encoder_outputs['physics_constraints'],
                targets.get('planet_params'),
                epoch
            )
    ...
    def _compute_consistency_loss(self, individual_features: Dict[str, torch.Tensor]) -> torch.Tensor:
    ...
    def _compute_gradient_smoothness(self, pred: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        if pred.dim() >= 4:  # [batch, channels, height, width, ...]
            ...
            grad_loss = (self.mse_loss(grad_pred_h, grad_target_h) + 
                        self.mse_loss(grad_pred_w, grad_target_w)) / 2
...
class MultiModalTrainer(pl.LightningModule):
    ...
    def training_step(self, batch: MultiModalBatch, batch_idx: int) -> torch.Tensor:
        ...
        # Log losses
        for name, loss in losses.items():
            self.log(f'train/{name}_loss', loss, on_step=True, on_epoch=True, prog_bar=(name == 'total'))
    ...
    def validation_step(self, batch: MultiModalBatch, batch_idx: int) -> torch.Tensor:
        ...
        # Log losses
        for name, loss in losses.items():
            self.log(f'val/{name}_loss', loss, on_step=False, on_epoch=True, prog_bar=(name == 'total'))
    ...
    def _batch_to_dict(self, batch: MultiModalBatch) -> Dict[str, Any]:
        ...
        batch_data = {
            'planet_params': batch.planet_params
        }
...
def create_trainer(encoder_config: EncoderConfig, 
                  training_config: TrainingConfig) -> Tuple[MultiModalTrainer, pl.Trainer]:
...
if __name__ == "__main__":
    ...
    def test_training_workflow():
        ...
        train_loader, val_loader, _ = create_multimodal_dataloaders(
            dataloader_config, mock_storage
        )
