...
class EnhancedSSLCertificateManager:
    ...
    async def test_ssl_configuration(
        self, url: str, config: SSLConfiguration
            ...
            async with aiohttp.ClientSession(
                ...
                async with session.head(url) as response:
                    if response.status < 400:
                        logger.info(
                            f"SSL test successful for {domain} with {config.issue_type or 'default'} config"
                        )
                        return True, None
                    else:
    ...
    async def find_working_ssl_config(self, url: str) -> Optional[SSLConfiguration]:
        ...
        for config in test_configs:
            try:
                ...
                if success:
                    logger.info(
                        f"Found working SSL config for {domain}: {config.issue_type or 'default'}"
                    )
    ...
    async def get_optimized_session(self, url: str) -> aiohttp.ClientSession:
        ...
        logger.info(
            f"Created optimized session for {domain} with SSL config: {config.issue_type or 'default'}"
        )
        return session
    ...
    async def validate_all_data_sources(self, data_sources: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        success_rate = (
            validation_results["successful_ssl_configs"] / validation_results["total_sources"]
        ) * 100

        logger.info(f"SSL validation complete: {success_rate:.1f}% success rate")
        logger.info(
            f"Working SSL configs: {validation_results['successful_ssl_configs']}/{validation_results['total_sources']}"
        )
        ...
        try:
            ...
            if config:
                ...
                result["fallback_used"] = config.fallback_enabled and (
                    not config.verify_ssl
                    or config.allow_self_signed
                    or config.cert_reqs == ssl.CERT_NONE
                )
