...
class URLManager:
    ...
    def _detect_geographic_region(self) -> GeographicRegion:
        ...
        try:
            ...
            logger.debug(
                f"Geographic region detection failed: {e}"
    ...
    async def get_optimal_url(
        self, source_name: str, endpoint: str = "", prefer_region: Optional[GeographicRegion] = None
    ) -> Optional[str]:
        """
    ...
    def _build_candidate_urls(
        self, registry: DataSourceRegistry, region: GeographicRegion, endpoint: str
    ) -> List[Tuple[int, str]]:
                    ...
                    if response.status == expected_status:
                        ...
                        if "content_validation" in health_config:
                            ...
                            if not self._validate_content(
                                content, health_config["content_validation"]
                            ):
                                return False
                        ...
                        self._update_url_health(
                            url, URLStatus.FAILED, response_time, f"HTTP {response.status}"
                        )
                        return False
    ...
    async def _discover_alternative_url(self, source_name: str, endpoint: str) -> Optional[str]:
        ...
        if "nasa" in base_domain:
            patterns.extend(
                [f"https://archive.stsci.edu", f"https://mast.stsci.edu", f"https://data.nasa.gov"]
            )
        elif "ncbi" in base_domain:
            patterns.extend([f"https://ftp-private.ncbi.nlm.nih.gov", f"https://ftp.ncbi.nih.gov"])
    ...
    async def _run_health_checks(self):
        ...
        logger.info(
            f"Health check completed: {successful_checks}/{total_checks} "
        ...
        for source_name in self.registries.keys():
            try:
                ...
                for mirror_url in registry.mirror_urls:
                    ...
                    mirror_results.append({"url": mirror_url, "valid": mirror_valid})
                ...
                validation_results[source_name] = {
                    ...
                    "overall_status": (
                        "healthy"
                        if primary_valid or any(m["valid"] for m in mirror_results)
                        else "failed"
                    ),
                }

            except Exception as e:
                validation_results[source_name] = {"error": str(e), "overall_status": "error"}
