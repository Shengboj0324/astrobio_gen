...
try:
    from .enhanced_ssl_certificate_manager import ssl_manager, get_ssl_optimized_session, get_ssl_optimized_requests_session
...
def configure_ssl_for_external_apis():
    ...
    if ENHANCED_SSL_AVAILABLE:
        ...
        logger.warning("Enhanced SSL Certificate Manager not available - using fallback SSL configuration")
...
def check_ssl_configuration():
    ...
    if ENHANCED_SSL_AVAILABLE and ssl_manager:
        try:
            ...
            logger.info(f"Enhanced SSL manager: {ssl_status['total_domain_configs']} domains configured")
        except Exception as e:
