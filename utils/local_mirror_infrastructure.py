...
class LocalMirrorInfrastructure:
    ...
    def __init__(self, config_path: str = "config/config.yaml"):
        ...
        logger.info(
            f"Local Mirror Infrastructure initialized with {len(self.mirror_configs)} sources"
        )
    ...
    async def _schedule_initial_syncs(self):
        ...
        try:
            ...
            critical_sources = [
                name for name, config in self.mirror_configs.items() if config.priority == 1
            ]
    ...
    async def sync_source(self, source_name: str, force: bool = False) -> Optional[SyncJob]:
        ...
        try:
            ...
            logger.info(
                f"Sync completed for {source_name}: {sync_job.files_synced} files, "
    ...
    async def _perform_sync(
        self, config: MirrorConfig, sync_job: SyncJob, bucket_name: str, s3_path: str
    ):
    ...
    async def _sync_nasa_exoplanet_archive(
        self, config: MirrorConfig, sync_job: SyncJob, bucket_name: str, s3_path: str
    ):
                ...
                try:
                    ...
                    async with session.get(dataset["url"], timeout=300) as response:
                        if response.status == 200:
                            ...
                            error_msg = (
                                f"Failed to download {dataset['name']}: HTTP {response.status}"
                            )
                                ...
                                async with session.get(kgml_url, timeout=30) as kgml_response:
                                    if kgml_response.status == 200:
                                        ...
                                        if sync_job.files_synced % 10 == 0:
                                            logger.info(
                                                f"Synced {sync_job.files_synced} KEGG pathways"
                                            )
    ...
    async def _sync_generic_source(
        self, config: MirrorConfig, sync_job: SyncJob, bucket_name: str, s3_path: str
    ):
    ...
    async def check_mirror_health(self, source_name: Optional[str] = None) -> Dict[str, Any]:
        ...
        for src_name in sources_to_check:
            ...
            try:
                ...
                health_results[src_name] = {"status": "error", "error": str(e)}
    ...
    def _check_mirror_availability(self, bucket_name: str, mirror_path: str) -> bool:
        ...
        try:
            response = self.s3_client.list_objects_v2(
                Bucket=bucket_name, Prefix=mirror_path, MaxKeys=1
            )
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                """,
                    (
                        ...
                        (
                            health_data.get("source_size_mb", 0) * 1024 * 1024
                            if health_data.get("source_size_mb")
                            else None
                        ),
                        (
                            health_data.get("mirror_size_mb", 0) * 1024 * 1024
                            if health_data.get("mirror_size_mb")
                            else None
                        ),
        ...
        for source_name, config in self.mirror_configs.items():
            if config.sync_frequency == "hourly":
                ...
                schedule.every().day.at("02:00").do(
                    lambda: asyncio.create_task(self.sync_source(source_name))
                )
