...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("first_round_data_capture.log"), logging.StreamHandler()],
)
...
class FirstRoundDataCapture:
        ...
        try:
            ...
            if self.config["enable_comprehensive_acquisition"]:
                ...
                comprehensive_results = (
                    await self.comprehensive_acquisition.run_comprehensive_acquisition(
                ...
                logger.info(
                    f"Phase 1 completed: {comprehensive_results['successful_domains']} domains"
                )
            ...
            if self.config["enable_real_data_sources"]:
                ...
                scraping_results = await self.real_data_scraper.scrape_all_sources(
                    sources=scraper_sources, max_size_gb=self.config["max_download_size_gb"]
                )
            ...
            if self.config["enable_quality_validation"]:
                ...
                logger.info(
                    f"Phase 3 completed: {quality_results['nasa_grade_datasets']} NASA-grade datasets"
                )
            ...
            if self.config["enable_metadata_tracking"]:
                ...
                logger.info(
                    f"Phase 4 completed: {metadata_results['total_datasets']} datasets registered"
                )
    ...
    def _update_progress(self, phase_results: Dict[str, Any]) -> None:
        ...
        # Log progress
        logger.info(
            f"Progress Update: {self.progress['completed_domains']}/{self.progress['total_domains']} domains, "
    ...
    async def _run_quality_validation(self) -> Dict[str, Any]:
        ...
        if validation_results["quality_scores"]:
            avg_quality = sum(validation_results["quality_scores"].values()) / len(
                validation_results["quality_scores"]
            )
            validation_results["average_quality_score"] = avg_quality
            validation_results["nasa_compliance_rate"] = (
                validation_results["nasa_grade_datasets"]
    ...
    async def _run_metadata_integration(self) -> Dict[str, Any]:
        ...
        integration_results = {"total_datasets": 0, "cross_references": 0, "metadata_errors": []}
        ...
        try:
            ...
            integration_results["cross_references"] = min(
                integration_results["total_datasets"] // 10, 100
            )
        ...
        summary = {
            ...
            "quality_metrics": {
                "nasa_compliance_rate": (
                    self.progress["nasa_grade_datasets"]
                    / max(self.progress["completed_domains"], 1)
                )
                * 100,
                "average_quality_score": self.progress["quality_score"],
                "total_validated_datasets": results.get("quality_validation", {}).get(
                    "total_domains_validated", 0
                ),
                "quality_threshold_met": self.progress["quality_score"]
            ...
            "storage_metrics": {
                ...
                "utilization_percent": (
                    self.progress["downloaded_size_tb"] / self.config["max_storage_tb"]
                )
                * 100,
                "remaining_capacity_tb": self.config["max_storage_tb"]
                - self.progress["downloaded_size_tb"],
            },
            # Scientific coverage
            "scientific_coverage": {
    ...
    def _generate_error_summary(
        self, error: str, partial_results: Dict[str, Any]
    ) -> Dict[str, Any]:
...
def parse_arguments():
    ...
    parser.add_argument(
        "--max-storage-tb", type=float, default=50.0, help="Maximum storage in TB (default: 50.0)"
    )
    parser.add_argument(
        "--max-download-gb",
        type=float,
    ...
    parser.add_argument(
        "--quality-threshold", type=float, default=0.90, help="Quality threshold (default: 0.90)"
    )
    parser.add_argument(
        "--nasa-grade-threshold",
        type=float,
        default=0.92,
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Run in dry-run mode (no actual downloads)"
    )
