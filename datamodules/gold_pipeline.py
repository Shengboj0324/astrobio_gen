...
class ROCKE3DDataset(Dataset):
    ...
    def __init__(self, data_dir: Path, mode: str = "scalar", cache_dir: Optional[Path] = None):
            ...
            try:
                ...
                with xr.open_dataset(row["file_path"]) as ds:
                    ...
                    if self.mode == "scalar":
                        ...
                        target = {"temperature_field": temp_field, "humidity_field": humidity_field}
            ...
            if self.mode == "scalar":
                ...
                target_dict = {
                    "temperature_field": torch.tensor(
                        grp["temperature_field"][...], dtype=torch.float32
                    ),
...
class NASAExoplanetDataset(Dataset):
    ...
    def _preprocess(self):
        ...
        mask = (
            (self.data["pl_rade"] > 0.5)
            & (self.data["pl_rade"] < 2.0)
            & (self.data["pl_insol"] > 0.2)
            & (self.data["pl_insol"] < 2.0)
        ...
        if stage == "fit" or stage is None:
            ...
            if (self.rocke3d_dir / "rocke3d_metadata.csv").exists():
                ...
                planet_data, targets = create_synthetic_climate_data(
                    self.config["data"]["synthetic_size"], mode=self.mode
                )
        ...
        if stage == "test" or stage is None:
            ...
            if benchmark_file.exists():
                ...
                for name, data in benchmarks.items():
                    ...
                    if self.mode == "scalar":
                        targets = {
                            "habitability": torch.tensor(
                                [data["expected_habitability"]], dtype=torch.float32
                            ),
                            "surface_temp": torch.tensor(
                                [data["expected_temp"]], dtype=torch.float32
                            ),
                            "atmospheric_pressure": torch.tensor(
