...
class IntegrationValidator:
    ...
    async def _validate_data_acquisition(self):
        ...
        successful_integrations = sum(
            1
            for status in acquisition_status.values()
            if status.get("status", "").startswith("[OK]")
        )

        logger.info(
            f"[OK] Data Acquisition: {successful_integrations}/{len(acquisition_status)} modules integrated"
        )
    ...
    async def _validate_surrogate_models(self):
        ...
        try:
            ...
            training_data = await data_manager.acquire_training_data(
                n_samples=10, data_types=["planetary"]
            )
    ...
    async def _validate_datacube_system(self):
        ...
        try:
            ...
            try:
                ...
                logger.debug(
                    f"CubeDM datamodule import failed: {e}"
    ...
    def _generate_final_assessment(self):
        ...
        success_rate = (
            (successful_components / total_components * 100) if total_components > 0 else 0
        )
