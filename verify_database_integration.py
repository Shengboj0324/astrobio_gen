...
def test_centralized_config():
    ...
    try:
        ...
        logger.info(
            f"✅ Database manager initialized with {len(manager.list_databases())} databases"
        )

        # Verify all databases
        verification = verify_database_system()
        logger.info(
            f"✅ Database verification completed: {verification['accessible']}/{verification['total_databases']} accessible"
        )
...
def test_comprehensive_acquisition_integration():
    ...
    try:
        ...
        spec = importlib.util.spec_from_file_location(
            "comprehensive", "data_build/comprehensive_multi_domain_acquisition.py"
        )
...
def generate_database_report():
    ...
    try:
        ...
        accessible_dbs = [
            db for db, info in report["verification"]["database_info"].items() if info["exists"]
        ]

        total_size_mb = sum(
            info["size_mb"]
            for info in report["verification"]["database_info"].values()
            if info["exists"]
        )
        ...
        report["performance"] = {
            ...
            "largest_database": max(
                [
                    (db, info["size_mb"])
                    for db, info in report["verification"]["database_info"].items()
                    if info["exists"]
                ],
                key=lambda x: x[1],
                default=("none", 0),
        ...
        if len(accessible_dbs) < len(manager.list_databases()):
            report["recommendations"].append(
                "Run comprehensive data acquisition to create missing databases"
            )
        ...
        if not accessible_dbs:
            report["recommendations"].append(
                "Initialize database system by running basic data acquisition"
            )
...
def main():
    ...
    if report:
        ...
        logger.info(
            f"Database Status: {verification.get('accessible', 0)}/{verification.get('total_databases', 0)} accessible"
        )
