...
from scripts.train_fusion_dummy import schema as FUSION_SCHEMA
from scripts.train_fusion_dummy import to_tensor
        ...
        if hasattr(self.model, "compute_total_loss"):
            ...
            losses = {
                "total_loss": torch.nn.functional.mse_loss(
                    outputs.get("predictions", outputs), targets
                )
            }
...
def create_synthetic_climate_data(n_samples: int = 1000, mode: str = "scalar") -> tuple:
    ...
    if mode == "scalar":
        ...
        habitability = 1.0 / (
            1.0 + np.exp(-(planet_params[:, 3] - 1.0) * 5)
        )  # Sigmoid around Earth-like
        habitability *= 1.0 / (
            1.0 + np.exp(-np.abs(planet_params[:, 0] - 1.0) * 5)
        )  # Penalty for non-Earth-size
...
def parse_enhanced_args():
    ...
    parser = argparse.ArgumentParser(
        description="Enhanced Training Script for Astrobiology Platform"
    )
    ...
    parser.add_argument(
        "--use-mixed-precision", action="store_true", help="Use mixed precision training"
    )
    ...
    parser.add_argument(
        "--episodes", type=int, default=1000, help="Number of episodes for meta-learning"
    )

    parser.add_argument(
        "--support-shots", type=int, default=5, help="Number of support shots for meta-learning"
    )

    parser.add_argument(
        "--query-shots", type=int, default=15, help="Number of query shots for meta-learning"
    )

    # Federated learning parameters
    parser.add_argument(
        "--participants", type=int, default=10, help="Number of participants for federated learning"
    )
    ...
    parser.add_argument(
        "--search-epochs", type=int, default=50, help="Number of epochs for architecture search"
    )

    parser.add_argument(
        "--search-space-size", type=int, default=1000, help="Size of architecture search space"
    )
    ...
    parser.add_argument(
        "--use-customer-data", action="store_true", help="Use customer data treatment"
    )
    ...
    # Legacy support
    parser.add_argument("--legacy", action="store_true", help="Use legacy training mode")
...
def get_model_config(model_name: str) -> Dict[str, Any]:
    ...
    configs = {
        ...
        "graph_vae": {"latent": 8},
        "fusion": {"hidden_dim": 64, "num_layers": 3},
        "surrogate": {
            "dim": 256,
            "depth": 8,
            "heads": 8,
...
async def main_legacy(args) -> Dict[str, Any]:
    ...
    # Setup logging
    if (
        cfg.get("logging", {}).get("use_wandb", False)
        and WANDB_AVAILABLE
        and WandbLogger is not None
    ):
        logger = WandbLogger(
    ...
    if cfg["model"]["type"] == "graph_vae":
        ...
        feat_tensors = [
            (
                to_tensor(df[c]).float()
                if i == 0
                else (
                    torch.tensor(df[c].values)
                    if i == 1
                    else torch.tensor(np.stack(df[c].values)).float()
                )
            )
        ...
        planet_data, targets = create_synthetic_climate_data(
            cfg["data"]["synthetic_size"], mode=mode
        )
        ...
        train_dl = DataLoader(
            train_ds, batch_size=cfg["trainer"]["batch_size"], shuffle=True, num_workers=4
        )
        val_dl = DataLoader(
            val_ds, batch_size=cfg["trainer"]["batch_size"], shuffle=False, num_workers=4
        )
    ...
    results = {
        ...
        "best_loss": float(
            trainer.callback_metrics.get(
                "val/total_loss", trainer.callback_metrics.get("loss", 0.0)
            )
        ),
